...
Private Declare Function mixerGetLineControls Lib "winmm.dll" Alias "mixerGetLineControlsA" (ByVal hmxobj As Long, pmxic As MIXERLINECONTROLS, ByVal fdwControls As Long) As Long
...
RetValue = mixerGetLineInfo(hmx, uMixerLine, MIXER_GETLINEINFOF_COMPONENTTYPE)
If RetValue <> MMSYSERR_NOERROR Then GoTo error

' Initialize MIXERLINECONTROLS strucure and
' call mixerGetLineCdntrols
...
RetValue = mixerGetLineControls(hmx, uMixerLineControls, MIXER_GETLINECONTROLSF_ONEBYTYPE)
If RetValue <> MMSYSERR_NOERROR Then GoTo error

' Copy data buffer into the uMixerControl structure
Call CopyMemory(uMixerControl, ByVal uMixerLineControls.pamxctrl, Len(uMixerControl))
GlobalFree hmem
hmem = 0
uDetails.item = 0
...
uUnsigned.dwValue = CLng((VolumeLevel * uMixerControl.IMaximum) / 100)
CopyMemory ByVal uDetails.paDetails, uUnsigned, Len(uUnsigned)

' Set new volume level
RetValue = mixerSetControlDetails(hmx, uDetails, MIXER_SETCONTROLDETAILSF_VALUE)
GlobalFree hmem
hmem = 0
If RetValue <> MMSYSERR_NOERROR Then GoTo error
