...
Begin VB.UserControl vpnWMSOPT_MOVE_admi 
   ClientHeight    =   3600
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   4800
   ...
   Begin VB.Frame pnlWMSOPT_MOVE 
      ...
      Begin GridEX20.GridEX gridWMSOPT_MOVE 
         ...
         IntProp7        =   0
         ColumnsCount    =   2
         Column(1)       =   "vpnWMSOPT_MOVE_admi.ctx":0000
         Column(2)       =   "vpnWMSOPT_MOVE_admi.ctx":00C8
         FormatStylesCount=   5
         FormatStyle(1)  =   "vpnWMSOPT_MOVE_admi.ctx":016C
         FormatStyle(2)  =   "vpnWMSOPT_MOVE_admi.ctx":02C8
         FormatStyle(3)  =   "vpnWMSOPT_MOVE_admi.ctx":0378
         FormatStyle(4)  =   "vpnWMSOPT_MOVE_admi.ctx":042C
         FormatStyle(5)  =   "vpnWMSOPT_MOVE_admi.ctx":0504
         ImageCount      =   0
         PrinterProperties=   "vpnWMSOPT_MOVE_admi.ctx":05BC
      End
   ...
   End
End
Attribute VB_Name = "vpnWMSOPT_MOVE_admi"
...
If gridWMSOPT_MOVE.Row > 0 Then
 If gridWMSOPT_MOVE.RowIndex(gridWMSOPT_MOVE.Row) > 0 Then
    ...
    bm = gridWMSOPT_MOVE.RowBookmark(gridWMSOPT_MOVE.RowIndex(gridWMSOPT_MOVE.Row))
    Set u = Item.FindRowObject(Right(bm, Len(bm) - 38), Left(bm, 38))
    Set frmWMSOPT_MOVE_admi.Item = u
again:     frmWMSOPT_MOVE_admi.NotFirstTime = False
    frmWMSOPT_MOVE_admi.OnInit
    frmWMSOPT_MOVE_admi.Show vbModal
    If frmWMSOPT_MOVE_admi.OK Then
    On Error Resume Next
    err.Clear
    u.Save
    If err.Number <> 0 Then
      ...
      GoTo again
    End If
    gridWMSOPT_MOVE.RefreshRowBookmark bm
    Else
     u.Refresh
    End If
 End If
...
Private Sub cmdWMSOPT_MOVEDel_Click()
  On Error Resume Next
  gridWMSOPT_MOVE.Delete
End Sub
Private Sub cmdWMSOPT_MOVEAdd_Click()
  On Error Resume Next
      Dim u As Object
      Set u = Item.WMSOPT_MOVE.Add()
...
again:       frmWMSOPT_MOVE_admi.NotFirstTime = False
      ...
      If frmWMSOPT_MOVE_admi.OK Then
        ...
        gridWMSOPT_MOVE.ItemCount = Item.WMSOPT_MOVE.Count
        gridWMSOPT_MOVE.Refresh
        gridWMSOPT_MOVE.RefreshGroups
        gridWMSOPT_MOVE.RefreshSort
        gridWMSOPT_MOVE.MoveToBookmark u.ID & "WMSOPT_MOVE"
      Else
        Item.WMSOPT_MOVE.Delete u.ID
        Item.WMSOPT_MOVE.Remove u.ID
      End If
      Exit Sub
bye:
   MsgBox err.Description, vbOKOnly + vbExclamation, "Создание"
...
Private Sub gridWMSOPT_MOVE_BeforeDeleteEX(ByVal RowIndex As Long, ByVal Bookmark As Variant, ByVal Cancel As GridEX20.JSRetBoolean)
  If gridWMSOPT_MOVE.ItemCount = 0 Then Exit Sub
  Dim u As Object
  Set u = Item.FindRowObject("WMSOPT_MOVE", Left(Bookmark, 38))
