...
Begin VB.UserControl vpnWMSOPT_MOVE_ 
   ClientHeight    =   3600
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   4800
   ...
   Begin VB.Frame pnlWMSOPT_MOVE 
      ...
      Begin GridEX20.GridEX gridWMSOPT_MOVE 
         ...
         IntProp7        =   0
         ColumnsCount    =   2
         Column(1)       =   "vpnWMSOPT_MOVE_.ctx":0000
         Column(2)       =   "vpnWMSOPT_MOVE_.ctx":00C8
         FormatStylesCount=   5
         FormatStyle(1)  =   "vpnWMSOPT_MOVE_.ctx":016C
         FormatStyle(2)  =   "vpnWMSOPT_MOVE_.ctx":02C8
         FormatStyle(3)  =   "vpnWMSOPT_MOVE_.ctx":0378
         FormatStyle(4)  =   "vpnWMSOPT_MOVE_.ctx":042C
         FormatStyle(5)  =   "vpnWMSOPT_MOVE_.ctx":0504
         ImageCount      =   0
         PrinterProperties=   "vpnWMSOPT_MOVE_.ctx":05BC
      End
   ...
   End
End
Attribute VB_Name = "vpnWMSOPT_MOVE_"
...
If gridWMSOPT_MOVE.Row > 0 Then
 If gridWMSOPT_MOVE.RowIndex(gridWMSOPT_MOVE.Row) > 0 Then
    ...
    bm = gridWMSOPT_MOVE.RowBookmark(gridWMSOPT_MOVE.RowIndex(gridWMSOPT_MOVE.Row))
    Set u = Item.FindRowObject(Right(bm, Len(bm) - 38), Left(bm, 38))
    Set frmWMSOPT_MOVE_.Item = u
again:     frmWMSOPT_MOVE_.NotFirstTime = False
    frmWMSOPT_MOVE_.OnInit
    frmWMSOPT_MOVE_.Show vbModal
    If frmWMSOPT_MOVE_.OK Then
    On Error Resume Next
    err.Clear
    u.Save
