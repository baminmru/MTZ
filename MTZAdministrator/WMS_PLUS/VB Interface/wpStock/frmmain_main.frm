...
Begin VB.Form frmmain_main 
   ...
   Tag             =   "Card.ICO"
   Begin wpStockGUI.ctlmain_main ctl 
      Height          =   1500
      Left            =   0
      TabIndex        =   0
      Top             =   0
...
Private Sub LoadFromSkin()
  ...
  Dim ff As Long, buf As String
  ff = FreeFile
  Open s & "MainForm" & "wpStock" For Input As #ff
  buf = Input(LOF(ff), ff)
  Close #ff
...
Private Sub SaveToSkin()
  ...
  Dim ff As Long
  ff = FreeFile
  Kill s & "MainForm" & "wpStock"
  Open s & "MainForm" & "wpStock" For Output As #ff
  buf = "FormTag:" & Me.Tag
