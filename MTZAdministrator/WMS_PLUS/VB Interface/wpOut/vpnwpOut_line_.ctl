...
Begin VB.UserControl vpnwpOut_line_ 
   ClientHeight    =   3600
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   4800
   ...
   Begin VB.Frame pnlwpOut_line 
      ...
      Begin GridEX20.GridEX gridwpOut_line 
         ...
         IntProp7        =   0
         ColumnsCount    =   2
         Column(1)       =   "vpnwpOut_line_.ctx":0000
         Column(2)       =   "vpnwpOut_line_.ctx":00C8
         FormatStylesCount=   5
         FormatStyle(1)  =   "vpnwpOut_line_.ctx":016C
         FormatStyle(2)  =   "vpnwpOut_line_.ctx":02C8
         FormatStyle(3)  =   "vpnwpOut_line_.ctx":0378
         FormatStyle(4)  =   "vpnwpOut_line_.ctx":042C
         FormatStyle(5)  =   "vpnwpOut_line_.ctx":0504
         ImageCount      =   0
         PrinterProperties=   "vpnwpOut_line_.ctx":05BC
      End
   ...
   End
End
Attribute VB_Name = "vpnwpOut_line_"
...
If gridwpOut_line.Row > 0 Then
 If gridwpOut_line.RowIndex(gridwpOut_line.Row) > 0 Then
    ...
    bm = gridwpOut_line.RowBookmark(gridwpOut_line.RowIndex(gridwpOut_line.Row))
    Set u = Item.FindRowObject(Right(bm, Len(bm) - 38), Left(bm, 38))
    Set frmwpOut_line_.Item = u
again:     frmwpOut_line_.NotFirstTime = False
    frmwpOut_line_.OnInit
    frmwpOut_line_.Show vbModal
    If frmwpOut_line_.OK Then
    On Error Resume Next
    err.Clear
    u.Save
