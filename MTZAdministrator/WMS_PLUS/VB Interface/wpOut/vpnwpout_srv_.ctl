...
Begin VB.UserControl vpnwpout_srv_ 
   ClientHeight    =   3600
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   4800
   ...
   Begin VB.Frame pnlwpout_srv 
      ...
      Begin GridEX20.GridEX gridwpout_srv 
         ...
         IntProp7        =   0
         ColumnsCount    =   2
         Column(1)       =   "vpnwpout_srv_.ctx":0000
         Column(2)       =   "vpnwpout_srv_.ctx":00C8
         FormatStylesCount=   5
         FormatStyle(1)  =   "vpnwpout_srv_.ctx":016C
         FormatStyle(2)  =   "vpnwpout_srv_.ctx":02C8
         FormatStyle(3)  =   "vpnwpout_srv_.ctx":0378
         FormatStyle(4)  =   "vpnwpout_srv_.ctx":042C
         FormatStyle(5)  =   "vpnwpout_srv_.ctx":0504
         ImageCount      =   0
         PrinterProperties=   "vpnwpout_srv_.ctx":05BC
      End
   ...
   End
End
Attribute VB_Name = "vpnwpout_srv_"
...
If gridwpout_srv.Row > 0 Then
 If gridwpout_srv.RowIndex(gridwpout_srv.Row) > 0 Then
    ...
    bm = gridwpout_srv.RowBookmark(gridwpout_srv.RowIndex(gridwpout_srv.Row))
    Set u = Item.FindRowObject(Right(bm, Len(bm) - 38), Left(bm, 38))
    Set frmwpout_srv_.Item = u
again:     frmwpout_srv_.NotFirstTime = False
    frmwpout_srv_.OnInit
    frmwpout_srv_.Show vbModal
    If frmwpout_srv_.OK Then
    On Error Resume Next
    err.Clear
    u.Save
