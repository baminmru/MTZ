...
Begin VB.UserControl ctlmain_main 
   ...
   Begin MSComctlLib.TabStrip ts 
      ...
      EndProperty
   End
   Begin wpPalGUI.vpnwpPal_def_main pnlwpPal_def 
      Height          =   1500
      Left            =   1500
      TabIndex        =   1
      Top             =   0
      ...
      _ExtentY        =   2646
   End
   Begin wpPalGUI.vpnwpPal_good_main pnlwpPal_good 
      Height          =   1500
      Left            =   3000
      TabIndex        =   2
      Top             =   0
...
If IsItOk Then IsItOk = pnlwpPal_def.IsOK
If IsItOk Then IsItOk = pnlwpPal_good.IsOK
IsOK = IsItOk
End Function
Private Sub ts_click()
  On Error Resume Next
  pnlwpPal_good.Visible = False
  pnlwpPal_def.Visible = False
   ...
   Case "wpPal_def"
     ...
     .Visible = True
     .ZOrder 0
     pnlwpPal_def.OnClick item, ParentForm
     End With
   Case "wpPal_good"
     With pnlwpPal_good
     .Top = ts.ClientTop
     ...
     .Visible = True
     .ZOrder 0
     pnlwpPal_good.OnClick item, ParentForm
     End With
     End Select
End Sub
...
ts.Tabs.item(1).Key = "wpPal_def"
pnlwpPal_def.OnInit item, ParentForm
Call ts.Tabs.Add(, "wpPal_good", "Товар на паллете")
pnlwpPal_good.OnInit item, ParentForm
  Set TSCustom = New MTZ_CUSTOMTAB.TabStripCustomizer
  TSCustom.Init ts, "wpPal", "ctlmain_main"
...
Private Sub UserControl_Terminate()
 ...
 pnlwpPal_def.CloseClass
 pnlwpPal_good.CloseClass
End Sub
...
Public Function IsChanged() As Boolean
  ...
  On Error Resume Next
m_IsChanged = m_IsChanged Or pnlwpPal_def.IsChanged
m_IsChanged = m_IsChanged Or pnlwpPal_good.IsChanged
...
Public Function SetupStatusMenu(m() As Object)
    ...
    m(i).Enabled = True
    m(i).Checked = False
    m(i).Tag = "{FF7951B6-035E-4ADB-8B66-00C72740E543}"
    i = i + 1
    m(i).Caption = "С грузом"
    m(i).Visible = True
    m(i).Enabled = True
    m(i).Checked = False
    m(i).Tag = "{5E67134F-3A06-4639-8993-ABC12AB96D00}"
    i = i + 1
    m(i).Caption = "Пустая"
    m(i).Visible = True
    m(i).Enabled = True
    m(i).Checked = False
    m(i).Tag = "{58B81E48-6D54-4F47-A237-EC4B72718D7D}"
End Function
...
Public Sub CheckStatusMenu(m() As Object)
    ...
    Next
    If Not item Is Nothing Then
      If item.Statusid = "{FF7951B6-035E-4ADB-8B66-00C72740E543}" Then
        For i = 1 To StatusMenuCount
        Next
      End If
      If item.Statusid = "{5E67134F-3A06-4639-8993-ABC12AB96D00}" Then
        For i = 1 To StatusMenuCount
        Next
      End If
      If item.Statusid = "{58B81E48-6D54-4F47-A237-EC4B72718D7D}" Then
        For i = 1 To StatusMenuCount
        Next
      End If
   End If
