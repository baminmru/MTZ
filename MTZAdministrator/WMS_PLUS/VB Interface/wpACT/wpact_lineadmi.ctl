...
Begin VB.UserControl wpact_lineadmi 
   ClientHeight    =   3600
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   4800
   ...
   Begin MTZ_PANEL.ScrolledWindow Panel 
      ...
      _ExtentX        =   1773
      _ExtentY        =   1773
      Begin MSMask.MaskEdBox txtQWrong 
         Height          =   300
         Left            =   300
         TabIndex        =   12
         ToolTipText     =   "Расхождение"
         Top             =   3225
         Width           =   1800
         _ExtentX        =   3175
         _ExtentY        =   529
         ...
         PromptChar      =   "_"
      End
      Begin MSMask.MaskEdBox txtQOut 
         Height          =   300
         Left            =   300
         TabIndex        =   10
         ToolTipText     =   "Отгружено"
         Top             =   2520
         Width           =   1800
         _ExtentX        =   3175
         _ExtentY        =   529
   ...
   End
End
Attribute VB_Name = "wpact_lineadmi"
...
Private Sub cmdedizm_MenuClick(ByVal sCaption As String)
          ...
          txtedizm = ""
End Sub
Private Sub txtQIn_Validate(cancel As Boolean)
If txtQIn.Text <> "" Then
 On Error Resume Next
  If Not IsNumeric(txtQIn.Text) Then
     cancel = True
     MsgBox "Ожидалось число", vbOKOnly + vbExclamation, "Внимание"
     txtQIn.SetFocus
  ElseIf Val(txtQIn.Text) < -922337203685478# Or Val(txtQIn.Text) > 922337203685478# Then
     cancel = True
...
End If
End Sub
Private Sub txtQIn_KeyPess(KeyAscii As Integer)
Dim s As String
s = "0123456789.,-" & Chr(8)
...
End Sub
Private Sub txtQOut_Validate(cancel As Boolean)
If txtQOut.Text <> "" Then
 On Error Resume Next
  If Not IsNumeric(txtQOut.Text) Then
     cancel = True
     MsgBox "Ожидалось число", vbOKOnly + vbExclamation, "Внимание"
     txtQOut.SetFocus
  ElseIf Val(txtQOut.Text) < -922337203685478# Or Val(txtQOut.Text) > 922337203685478# Then
     cancel = True
     MsgBox "Значение вне допустимого диапазона", vbOKOnly + vbExclamation, "Внимание"
     txtQOut.SetFocus
  End If
End If
End Sub
Private Sub txtQOut_KeyPess(KeyAscii As Integer)
Dim s As String
s = "0123456789.,-" & Chr(8)
If InStr(s, Chr(KeyAscii)) > 0 Then Exit Sub
KeyAscii = 0
...
End Sub
Private Sub txtQOut_Change()
  Changing

End Sub
Private Sub txtQWrong_Validate(cancel As Boolean)
If txtQWrong.Text <> "" Then
 On Error Resume Next
  If Not IsNumeric(txtQWrong.Text) Then
     cancel = True
     MsgBox "Ожидалось число", vbOKOnly + vbExclamation, "Внимание"
     txtQWrong.SetFocus
  ElseIf Val(txtQWrong.Text) < -922337203685478# Or Val(txtQWrong.Text) > 922337203685478# Then
     cancel = True
     MsgBox "Значение вне допустимого диапазона", vbOKOnly + vbExclamation, "Внимание"
     txtQWrong.SetFocus
  End If
End If
End Sub
Private Sub txtQWrong_KeyPess(KeyAscii As Integer)
Dim s As String
s = "0123456789.,-" & Chr(8)
If InStr(s, Chr(KeyAscii)) > 0 Then Exit Sub
KeyAscii = 0
...
If mIsOK Then mIsOK = IsSet(txtQIn.Text)
If mIsOK Then mIsOK = IsSet(txtQOut.Text)
If mIsOK Then mIsOK = IsSet(txtQWrong.Text)
  IsOK = mIsOK
End Function
...
End If
Item.QIn = CDbl(txtQIn)
Item.QOut = CDbl(txtQOut)
Item.QWrong = CDbl(txtQWrong)
 mIsChanged = False
 RaiseEvent Changed
End Sub
