...
Begin VB.UserControl vpnwpin_srv_ 
   ClientHeight    =   3600
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   4800
   ...
   Begin VB.Frame pnlwpin_srv 
      ...
      Begin GridEX20.GridEX gridwpin_srv 
         ...
         IntProp7        =   0
         ColumnsCount    =   2
         Column(1)       =   "vpnwpin_srv_.ctx":0000
         Column(2)       =   "vpnwpin_srv_.ctx":00C8
         FormatStylesCount=   5
         FormatStyle(1)  =   "vpnwpin_srv_.ctx":016C
         FormatStyle(2)  =   "vpnwpin_srv_.ctx":02C8
         FormatStyle(3)  =   "vpnwpin_srv_.ctx":0378
         FormatStyle(4)  =   "vpnwpin_srv_.ctx":042C
         FormatStyle(5)  =   "vpnwpin_srv_.ctx":0504
         ImageCount      =   0
         PrinterProperties=   "vpnwpin_srv_.ctx":05BC
      End
   ...
   End
End
Attribute VB_Name = "vpnwpin_srv_"
...
If gridwpin_srv.Row > 0 Then
 If gridwpin_srv.RowIndex(gridwpin_srv.Row) > 0 Then
    ...
    bm = gridwpin_srv.RowBookmark(gridwpin_srv.RowIndex(gridwpin_srv.Row))
    Set u = Item.FindRowObject(Right(bm, Len(bm) - 38), Left(bm, 38))
    Set frmwpin_srv_.Item = u
again:     frmwpin_srv_.NotFirstTime = False
    frmwpin_srv_.OnInit
    frmwpin_srv_.Show vbModal
    If frmwpin_srv_.OK Then
    On Error Resume Next
    err.Clear
    u.Save
