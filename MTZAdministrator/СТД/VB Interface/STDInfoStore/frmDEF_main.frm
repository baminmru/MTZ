...
Begin VB.Form frmDEF_main 
   ...
   Tag             =   "Card.ICO"
   Begin STDInfoStoreGUI.ctlDEF_main ctl 
      Height          =   1500
      Left            =   0
      TabIndex        =   0
      Top             =   0
...
' Главная форма редактирования режим DEF
            ...
            'If MsgBox("Закрыть без сохранения?", vbCritical + vbYesNo, Me.Caption) = vbYes Then
                If Not CheckIsOk(item) Then
                    If MsgBox("Поля не заполнены. Удалить документ?", vbCritical + vbYesNo, Me.Caption) = vbYes Then
                        On Error Resume Next
                        item.Manager.DeleteInstance item.ID
                        Host.EraseForm item.ID, "DEF", ModalMode
                        Set item = Nothing
                        Exit Sub
                    End If
                End If
...
Private Sub LoadFromSkin()
  ...
  Dim ff As Long, buf As String
  ff = FreeFile
  Open s & "MainForm" & "STDInfoStore" For Input As #ff
  buf = Input(LOF(ff), ff)
  Close #ff
...
Private Sub SaveToSkin()
  ...
  Dim ff As Long
  ff = FreeFile
  Kill s & "MainForm" & "STDInfoStore"
  Open s & "MainForm" & "STDInfoStore" For Output As #ff
  buf = "FormTag:" & Me.Tag
