...
Public Function CheckIsNew(o As Object)
    ...
    res = False
    CheckIsNew = True
    Set rs = o.MTZSession.GetData("select part.name as pName, part.parttype pType from part, objecttype where part.parentrowid is null and  part.parentstructrowid=objecttype.objecttypeid and objecttype.Name='" & o.TypeName & "'") ' and part.Parttype=0")
    ...
    While Not rs.EOF
        ...
        If rs!pType <> 3 Then
            ...
            If Not rsf Is Nothing Then
              If Not rsf.EOF Then
                If rsf!cnt > 0 Then
                  Set rsf = o.Application.MTZSession.GetData("select field.name as fName from field right join part on field.parentstructrowId=part.partid left join fieldtype on field.fieldtype = fieldtype.fieldtypeid where part.name='" & PartName & "' and IsAutoNumber=0 and fieldtype.typestyle<>5")
                  While Not rsf.EOF
