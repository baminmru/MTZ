VERSION 5.00
Begin VB.Form frmread_main 
   Caption         =   "Оптимизация старая"
   ClientHeight    =   3090
   ClientLeft      =   60
   ClientTop       =   750
   ClientWidth     =   4680
   ...
   Tag             =   "Card.ICO"
   Begin ITT2OPTGUI.ctlread_main ctl 
      Height          =   1500
      Left            =   0
      TabIndex        =   0
      Top             =   0
   ...
   End
End
Attribute VB_Name = "frmread_main"
...
' Главная форма редактирования режим read
...
Private Sub LoadFromSkin()
  ...
  Dim ff As Long, buf As String
  ff = FreeFile
  Open s & "MainForm" & "ITT2OPT" For Input As #ff
  buf = Input(LOF(ff), ff)
  Close #ff
...
Private Sub SaveToSkin()
  ...
  Dim ff As Long
  ff = FreeFile
  Kill s & "MainForm" & "ITT2OPT"
  Open s & "MainForm" & "ITT2OPT" For Output As #ff
  buf = "FormTag:" & Me.Tag
