...
Begin VB.UserControl vpnITTOPT_MOVE_main 
   ClientHeight    =   3600
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   4800
   ...
   Begin VB.Frame pnlITTOPT_MOVE 
      ...
      Begin GridEX20.GridEX gridITTOPT_MOVE 
         ...
         IntProp7        =   0
         ColumnsCount    =   2
         Column(1)       =   "vpnITTOPT_MOVE_main.ctx":0000
         Column(2)       =   "vpnITTOPT_MOVE_main.ctx":00C8
         FormatStylesCount=   5
         FormatStyle(1)  =   "vpnITTOPT_MOVE_main.ctx":016C
         FormatStyle(2)  =   "vpnITTOPT_MOVE_main.ctx":02C8
         FormatStyle(3)  =   "vpnITTOPT_MOVE_main.ctx":0378
         FormatStyle(4)  =   "vpnITTOPT_MOVE_main.ctx":042C
         FormatStyle(5)  =   "vpnITTOPT_MOVE_main.ctx":0504
         ImageCount      =   0
         PrinterProperties=   "vpnITTOPT_MOVE_main.ctx":05BC
      End
   ...
   End
End
Attribute VB_Name = "vpnITTOPT_MOVE_main"
...
'панель для работы со строками раздела Необходимые перемещения
...
Dim jset As Object 'MTZ_JSetup.Customizer
Set jset = CreateObject("MTZ_JSetup.Customizer")
jset.Preview gridITTOPT_MOVE, "Перемещения"
Set jset = Nothing
End Sub
...
If gridITTOPT_MOVE.Row > 0 Then
 If gridITTOPT_MOVE.RowIndex(gridITTOPT_MOVE.Row) > 0 Then
    ...
    bm = gridITTOPT_MOVE.RowBookmark(gridITTOPT_MOVE.RowIndex(gridITTOPT_MOVE.Row))
    Set u = Item.FindRowObject(Right(bm, Len(bm) - 38), Left(bm, 38))
    Set frmITTOPT_MOVE_main.Item = u
again:     frmITTOPT_MOVE_main.NotFirstTime = False
    frmITTOPT_MOVE_main.OnInit
    frmITTOPT_MOVE_main.Show vbModal
    If frmITTOPT_MOVE_main.OK Then
    On Error Resume Next
    err.Clear
    u.Save
