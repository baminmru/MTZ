...
Begin VB.Form frmmain_main 
   ...
   Tag             =   "Card.ICO"
   Begin ITTPRGUI.ctlmain_main ctl 
      Height          =   1500
      Left            =   0
      TabIndex        =   0
      Top             =   0
...
Private Sub LoadFromSkin()
  ...
  Dim ff As Long, buf As String
  ff = FreeFile
  Open s & "MainForm" & "ITTPR" For Input As #ff
  buf = Input(LOF(ff), ff)
  Close #ff
...
Private Sub SaveToSkin()
  ...
  Dim ff As Long
  ff = FreeFile
  Kill s & "MainForm" & "ITTPR"
  Open s & "MainForm" & "ITTPR" For Output As #ff
  buf = "FormTag:" & Me.Tag
