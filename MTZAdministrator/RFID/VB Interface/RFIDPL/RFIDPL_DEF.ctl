...
Begin VB.UserControl RFIDPL_DEF 
   ClientHeight    =   3600
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   4800
   ...
   Begin MTZ_PANEL.ScrolledWindow Panel 
      ...
      Begin VB.TextBox txtPalletPlaceRef 
         ...
         Width           =   2550
      End
      Begin MSComCtl2.DTPicker dtpLastOpDate 
         Height          =   300
         Left            =   3450
         TabIndex        =   25
         ToolTipText     =   "Дата последней операции"
         Top             =   1815
         Width           =   2250
         _ExtentX        =   3969
         _ExtentY        =   529
      ...
      Begin MSComCtl2.DTPicker dtpWeightingDate 
         ...
         Left            =   3450
         TabIndex        =   23
         ToolTipText     =   "Дата взвешивания паддона"
         Top             =   1110
         Width           =   2250
         _ExtentX        =   3969
         _ExtentY        =   529
      ...
      Begin MSMask.MaskEdBox txtOnePackageWeight 
         ...
         PromptChar      =   "_"
      End
      Begin MSMask.MaskEdBox txtPWeight 
         Height          =   300
         Left            =   300
         TabIndex        =   17
      ...
      Begin VB.Label lblWeightingDate 
         BackStyle       =   0  'Transparent
         Caption         =   "Дата взвешивания паддона:"
         ForeColor       =   &H00FF0000&
         Height          =   300
         Left            =   3450
         TabIndex        =   22
...
Attribute VB_HelpID = 355
Option Explicit



   Public item As Object
Attribute item.VB_VarHelpID = 390
   Private OnInit As Boolean
   Public Event Changed()
Attribute Changed.VB_HelpID = 360
...
Attribute IsChanged.VB_HelpID = 380
...
End Sub
Private Sub txtPWeight_Validate(Cancel As Boolean)
If txtPWeight.Text <> "" Then
 On Error Resume Next
  If Not IsNumeric(txtPWeight.Text) Then
     Cancel = True
     MsgBox "Ожидалось число", vbOKOnly + vbExclamation, "Внимание"
     txtPWeight.SetFocus
  ElseIf val(txtPWeight.Text) < -922337203685478# Or val(txtPWeight.Text) > 922337203685478# Then
     Cancel = True
     MsgBox "Значение вне допустимого диапазона", vbOKOnly + vbExclamation, "Внимание"
     txtPWeight.SetFocus
  End If
End If
End Sub
Private Sub txtPWeight_KeyPess(KeyAscii As Integer)
Dim s As String
s = "0123456789.,-" & Chr(8)
If InStr(s, Chr(KeyAscii)) > 0 Then Exit Sub
KeyAscii = 0
...
End Sub
Private Sub dtpLastOpDate_Change()
  Changing
...
Attribute InitPanel.VB_HelpID = 375
OnInit = True
...
End If
dtpLastOpDate = Now
If item.LastOpDate <> 0 Then
 dtpLastOpDate = item.LastOpDate
Else
 dtpLastOpDate.Value = Null
End If
...
item.OneBoxWeight = CDbl(txtOneBoxWeight)
  ...
  End If
  If IsNull(dtpLastOpDate) Then
    item.LastOpDate = 0
  Else
    item.LastOpDate = dtpLastOpDate.Value
  End If
...
Attribute OptimalSize.VB_HelpID = 395
...
Attribute OptimalY.VB_HelpID = 400
   Dim X As Single, Y As Single
   Panel.OptimalSize X, Y
   OptimalY = Y
 End Function
 Public Sub Customize()
Attribute Customize.VB_HelpID = 365
   Panel.Customize
 End Sub
 Public Property Get PanelCustomisationString() As String
Attribute PanelCustomisationString.VB_HelpID = 405
...
Attribute Enabled.VB_HelpID = 370
