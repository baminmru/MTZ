...
Begin VB.UserControl RFIDPL_OP 
   ClientHeight    =   3600
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   4800
   ...
   Begin MTZ_PANEL.ScrolledWindow Panel 
      ...
      Begin VB.TextBox txtPalletPlaceRef 
         ...
         Width           =   2550
      End
      Begin MSComCtl2.DTPicker dtpOpDate 
         Height          =   300
         Left            =   3450
         TabIndex        =   21
...
Attribute VB_HelpID = 475
Option Explicit



   Public item As Object
Attribute item.VB_VarHelpID = 510
   Private OnInit As Boolean
   Public Event Changed()
Attribute Changed.VB_HelpID = 480
...
Attribute IsChanged.VB_HelpID = 500
...
Private Sub cmdOpType_CLick()
  On Error Resume Next
        ...
        If item.Application.Manager.GetReferenceDialogEx2("RFIIDD_OPTYPE", ID, brief) Then
...
Attribute InitPanel.VB_HelpID = 495
OnInit = True
...
dtpOpDate = Now
If item.OpDate <> 0 Then
 dtpOpDate = item.OpDate
Else
 dtpOpDate.Value = Null
End If
...
End Sub
Public Sub Save()
Attribute Save.VB_HelpID = 530
If OnInit Then Exit Sub

If txtOpType.Tag <> "" Then
  Set item.OpType = item.Application.FindRowObject("RFIIDD_OPTYPE", txtOpType.Tag)
Else
  Set item.OpType = Nothing
End If
item.QryNum = txtQryNum
...
item.OneBoxWeight = CDbl(txtOneBoxWeight)
  If IsNull(dtpOpDate) Then
    item.OpDate = 0
  Else
    item.OpDate = dtpOpDate.Value
  End If
...
Attribute OptimalSize.VB_HelpID = 515
...
Attribute OptimalY.VB_HelpID = 520
   Dim X As Single, Y As Single
   Panel.OptimalSize X, Y
   OptimalY = Y
 End Function
 Public Sub Customize()
Attribute Customize.VB_HelpID = 485
   Panel.Customize
 End Sub
 Public Property Get PanelCustomisationString() As String
Attribute PanelCustomisationString.VB_HelpID = 525
...
Attribute Enabled.VB_HelpID = 490
