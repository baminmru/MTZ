...
Begin VB.Form frm_main 
   Caption         =   "Расширения функциональности"
   ClientHeight    =   3090
   ClientLeft      =   60
   ClientTop       =   750
   ClientWidth     =   4680
...
Private Sub LoadFromSkin()
  ...
  Dim ff As Long, buf As String
  ff = FreeFile
  Open s & "MainForm" & "MTZExt" For Input As #ff
  buf = Input(LOF(ff), ff)
  Close #ff
...
Private Sub SaveToSkin()
  ...
  Dim ff As Long
  ff = FreeFile
  Kill s & "MainForm" & "MTZExt"
  Open s & "MainForm" & "MTZExt" For Output As #ff
  buf = "FormTag:" & Me.Tag
