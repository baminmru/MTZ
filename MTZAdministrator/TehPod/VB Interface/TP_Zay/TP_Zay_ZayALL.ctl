...
Begin VB.UserControl TP_Zay_ZayALL 
   ClientHeight    =   3600
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   4800
   ...
   End
End
Attribute VB_Name = "TP_Zay_ZayALL"
...
If txtNum.Text <> "" Then
 On Error Resume Next
  If Not IsNumeric(txtNum.Text) Then
     ...
     txtNum.SetFocus
  ElseIf Val(txtNum.Text) <> CLng(Val(txtNum.Text)) Then
     cancel = True
...
End If
End Sub
Private Sub txtNum_KeyPess(KeyAscii As Integer)
Dim s As String
s = "0123456789.,-" & Chr(8)
...
Item.Num = CDbl(txtNum)
  If IsNull(dtpDataPriem) Then
    Item.DataPriem = 0
  Else
    Item.DataPriem = dtpDataPriem.Value
  End If
If txtZType.Tag <> "" Then
  Set Item.ZType = Item.Application.FindRowObject("TP_SPR_ZTYPE", txtZType.Tag)
Else
  Set Item.ZType = Nothing
End If
Item.FIO = txtFIO
If txtTheOperator.Tag <> "" Then
  Set Item.TheOperator = Item.Application.FindRowObject("TP_Spr_Oper", txtTheOperator.Tag)
Else
  Set Item.TheOperator = Nothing
End If
Item.Adres = txtAdres
If txtRayon.Tag <> "" Then
  Set Item.Rayon = Item.Application.FindRowObject("Rayon", txtRayon.Tag)
Else
  Set Item.Rayon = Nothing
End If
...
End If
If txtPortref.Tag <> "" Then
  Set Item.Portref = Item.Application.FindRowObject("TP_SPR_Port", txtPortref.Tag)
Else
  Set Item.Portref = Nothing
End If
Item.ABNumber = txtABNumber
Item.SN_STB = txtSN_STB
Item.MAC_STB = txtMAC_STB
Item.DopInfo = txtDopInfo
If txtKoordin.Tag <> "" Then
  Set Item.Koordin = Item.Application.FindRowObject("TP_Spr_Koor", txtKoordin.Tag)
Else
  Set Item.Koordin = Nothing
End If
...
End If
If txtDeviceType.Tag <> "" Then
  Set Item.DeviceType = Item.Application.FindRowObject("TP_SPR_MT", txtDeviceType.Tag)
Else
  Set Item.DeviceType = Nothing
End If
Item.VidanModemSer = txtVidanModemSer
If txtTarif.Tag <> "" Then
  Set Item.Tarif = Item.Application.FindRowObject("TP_Spr_Tar", txtTarif.Tag)
Else
  Set Item.Tarif = Nothing
End If
...
Item.OpisProblem = txtOpisProblem
 If cmbOtprVKEY.ListIndex >= 0 Then
   Item.OtprVKEY = cmbOtprVKEY.ItemData(cmbOtprVKEY.ListIndex)
 End If
  If IsNull(dtpDataOtpravVKEY) Then
    Item.DataOtpravVKEY = 0
  Else
    Item.DataOtpravVKEY = dtpDataOtpravVKEY.Value
  End If
 If cmbDopSogl.ListIndex >= 0 Then
   Item.DopSogl = cmbDopSogl.ItemData(cmbDopSogl.ListIndex)
 End If
Item.NumDogovor = txtNumDogovor
