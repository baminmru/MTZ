...
Begin VB.Form frmYING_main 
   ...
   Tag             =   "Card.ICO"
   Begin TP_ZayGUI.ctlYING_main ctl 
      Height          =   1500
      Left            =   0
      TabIndex        =   0
      Top             =   0
   ...
   End
End
Attribute VB_Name = "frmYING_main"
...
' Главная форма редактирования режим YING
            ...
            'If MsgBox("Закрыть без сохранения?", vbCritical + vbYesNo, Me.Caption) = vbYes Then
                If Not CheckIsOk(item) Then
                    If MsgBox("Поля не заполнены. Удалить документ?", vbCritical + vbYesNo, Me.Caption) = vbYes Then
                        On Error Resume Next
                        item.Manager.DeleteInstance item.ID
                        Host.EraseForm item.ID, "YING", ModalMode
                        Set item = Nothing
                        Exit Sub
                    End If
                End If
