...
Begin VB.UserControl vpnTP_ZAY_MSG_POL 
   ClientHeight    =   3600
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   4800
   ...
   Begin VB.Frame pnlTP_ZAY_MSG 
      ...
      Begin GridEX20.GridEX gridTP_ZAY_MSG 
         ...
         IntProp7        =   0
         ColumnsCount    =   2
         Column(1)       =   "vpnTP_ZAY_MSG_POL.ctx":0000
         Column(2)       =   "vpnTP_ZAY_MSG_POL.ctx":00C8
         FormatStylesCount=   5
         FormatStyle(1)  =   "vpnTP_ZAY_MSG_POL.ctx":016C
         FormatStyle(2)  =   "vpnTP_ZAY_MSG_POL.ctx":02C8
         FormatStyle(3)  =   "vpnTP_ZAY_MSG_POL.ctx":0378
         FormatStyle(4)  =   "vpnTP_ZAY_MSG_POL.ctx":042C
         FormatStyle(5)  =   "vpnTP_ZAY_MSG_POL.ctx":0504
         ImageCount      =   0
         PrinterProperties=   "vpnTP_ZAY_MSG_POL.ctx":05BC
      End
   ...
   End
End
Attribute VB_Name = "vpnTP_ZAY_MSG_POL"
...
If gridTP_ZAY_MSG.Row > 0 Then
 If gridTP_ZAY_MSG.RowIndex(gridTP_ZAY_MSG.Row) > 0 Then
    ...
    bm = gridTP_ZAY_MSG.RowBookmark(gridTP_ZAY_MSG.RowIndex(gridTP_ZAY_MSG.Row))
    Set u = item.FindRowObject(Right(bm, Len(bm) - 38), Left(bm, 38))
    Set frmTP_ZAY_MSG_POL.item = u
again:     frmTP_ZAY_MSG_POL.NotFirstTime = False
    frmTP_ZAY_MSG_POL.OnInit
    frmTP_ZAY_MSG_POL.Show vbModal
    If frmTP_ZAY_MSG_POL.OK Then
    On Error Resume Next
    err.Clear
    u.Save
