  ...
  namespace TP_MIN.ASPNET
{
  ...
  using TP_MIN;
  public partial class ucTabTP_MIN_DEF  : ucParent
  {
    // ƒÓ·‡‚ÎÂÌÌÓÂ ÛÍ‡ÏË
    ...
    public  TP_MIN.TP_MIN_DEF_col DataSource
    { 
      get
      {
        return (TP_MIN.TP_MIN_DEF_col)base.MKSNDataSource;
      }
    }
    ...
    // ¡ÀŒ  ”ƒ¿À≈Õ»ﬂ «¿œ»—» begin
      if (EVENTTARGET  == DeleteParameterName + "TP_MIN_DEF")
      {
    ...
    private void LoadData()
    {
      ...
      DataGrid_TP_MIN_DEF.Columns.Clear();
      DataGrid_TP_MIN_DEF.DataSource = DataView;
      this.DataBind();
     // change column headers here
      DataGrid_TP_MIN_DEF.Columns.FromKey("ID").ServerOnly = true;
      DataGrid_TP_MIN_DEF.Columns.FromKey("Brief").ServerOnly = true;

      if (DataGrid_TP_MIN_DEF.Columns.FromKey(EditColumnName) != null)
      {
          DataGrid_TP_MIN_DEF.Columns.FromKey(EditColumnName).AllowGroupBy = Infragistics.WebUI.UltraWebGrid.AllowGroupBy.No;
          DataGrid_TP_MIN_DEF.Columns.FromKey(EditColumnName).AllowRowFiltering = false;
          DataGrid_TP_MIN_DEF.Columns.FromKey(EditColumnName).Header.Caption = "Edit";
      }
      if (DataGrid_TP_MIN_DEF.Columns.FromKey(DeleteColumnName) != null)
      {
          DataGrid_TP_MIN_DEF.Columns.FromKey(DeleteColumnName).AllowGroupBy = Infragistics.WebUI.UltraWebGrid.AllowGroupBy.No;
          DataGrid_TP_MIN_DEF.Columns.FromKey(DeleteColumnName).AllowRowFiltering = false;
          DataGrid_TP_MIN_DEF.Columns.FromKey(DeleteColumnName).Header.Caption = "Delete";
      }

      DataGrid_TP_MIN_DEF.Columns.FromKey("TheDAte").Header.Caption = "ƒ‡Ú‡ ‡ÍÚ‡";
      DataGrid_TP_MIN_DEF.Columns.FromKey("ActNum").Header.Caption = "ÕÓÏÂ ‡ÍÚ‡";
      DataGrid_TP_MIN_DEF.Columns.FromKey("ClientReturns").Header.Caption = "¬ÓÁ‚‡Ú ÓÚ ‡·ÓÌÂÌÚÓ‚";
      DataGrid_TP_MIN_DEF.Columns.FromKey("ClientReturns_VAL").ServerOnly = true;
      DataGrid_TP_MIN_DEF.Columns.FromKey("NDS").Header.Caption = "—Ú‡‚Í‡ Õƒ—";
    ...
    protected string ClientScript()
    {
      ...
      Result += "{\n";
      Result += "__doPostBack('DeleteParameterNameTP_MIN_DEF',id);\n";
      Result += "}\n}\n";
      Result += "</script>\n";
      Result += "<script language=\"Javascript\">\n";
