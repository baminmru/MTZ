  ...
  namespace TP_MIN.ASPNET
{
  ...
  using TP_MIN;
  public partial class ucTabTP_MIN_LINE  : ucParent
  {
    // ƒÓ·‡‚ÎÂÌÌÓÂ ÛÍ‡ÏË
    ...
    public  TP_MIN.TP_MIN_LINE_col DataSource
    { 
      get
      {
        return (TP_MIN.TP_MIN_LINE_col)base.MKSNDataSource;
      }
    }
    ...
    // ¡ÀŒ  ”ƒ¿À≈Õ»ﬂ «¿œ»—» begin
      if (EVENTTARGET  == DeleteParameterName + "TP_MIN_LINE")
      {
    ...
    private void LoadData()
    {
      ...
      DataGrid_TP_MIN_LINE.Columns.Clear();
      DataGrid_TP_MIN_LINE.DataSource = DataView;
      this.DataBind();
     // change column headers here
      DataGrid_TP_MIN_LINE.Columns.FromKey("ID").ServerOnly = true;
      DataGrid_TP_MIN_LINE.Columns.FromKey("Brief").ServerOnly = true;

      if (DataGrid_TP_MIN_LINE.Columns.FromKey(EditColumnName) != null)
      {
          DataGrid_TP_MIN_LINE.Columns.FromKey(EditColumnName).AllowGroupBy = Infragistics.WebUI.UltraWebGrid.AllowGroupBy.No;
          DataGrid_TP_MIN_LINE.Columns.FromKey(EditColumnName).AllowRowFiltering = false;
          DataGrid_TP_MIN_LINE.Columns.FromKey(EditColumnName).Header.Caption = "Edit";
      }
      if (DataGrid_TP_MIN_LINE.Columns.FromKey(DeleteColumnName) != null)
      {
          DataGrid_TP_MIN_LINE.Columns.FromKey(DeleteColumnName).AllowGroupBy = Infragistics.WebUI.UltraWebGrid.AllowGroupBy.No;
          DataGrid_TP_MIN_LINE.Columns.FromKey(DeleteColumnName).AllowRowFiltering = false;
          DataGrid_TP_MIN_LINE.Columns.FromKey(DeleteColumnName).Header.Caption = "Delete";
      }

      DataGrid_TP_MIN_LINE.Columns.FromKey("Mtype").Header.Caption = "“ËÔ ÏÓ‰ÂÏ‡";
      DataGrid_TP_MIN_LINE.Columns.FromKey("Mtype_ID").ServerOnly = true;
      DataGrid_TP_MIN_LINE.Columns.FromKey("Quantity").Header.Caption = " ÓÎË˜ÂÒÚ‚Ó ÏÓ‰ÂÏÓ‚";
      DataGrid_TP_MIN_LINE.Columns.FromKey("IsBroken").Header.Caption = "ÕÂËÒÔ‡‚Ì˚Â";
      DataGrid_TP_MIN_LINE.Columns.FromKey("IsBroken_VAL").ServerOnly = true;
      DataGrid_TP_MIN_LINE.Columns.FromKey("PriceNDS").Header.Caption = "÷ÂÌ‡  Ò Õƒ—";
    ...
    protected string ClientScript()
    {
      ...
      Result += "{\n";
      Result += "__doPostBack('DeleteParameterNameTP_MIN_LINE',id);\n";
      Result += "}\n}\n";
      Result += "</script>\n";
      Result += "<script language=\"Javascript\">\n";
