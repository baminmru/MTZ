...
namespace TP_MIN.ASPNET
{
  ...
  /// <summary>
  ///    ucTP_MIN_DEF - форма редактирования раздела Акт приемки
  /// </summary>
  public partial class ucTP_MIN_DEF  : ucParent 
  {
    ...
    public TP_MIN.TP_MIN_DEF RowItem
    {
      get
      {
        return (TP_MIN.TP_MIN_DEF)base.MKSNRowItem;
      }
    }
   ...
   if (!(DataComponent.DenyVisible.IndexOf("ActNum") > -1)) {
          ...
          cmbClientReturns.Items.Clear();
          cmbClientReturns.Items.Insert(0, new System.Web.UI.WebControls.ListItem("Да", "-1"));
          cmbClientReturns.Items.Insert(0, new System.Web.UI.WebControls.ListItem("Нет", "0"));
          try {
          cmbClientReturns.SelectedValue = ((int)RowItem.ClientReturns).ToString();
          }catch{}
          cmbClientReturns.ToolTip = "Возврат от абонентов";
          if (ReadOnly || (DataComponent.DenyEdit.IndexOf("ClientReturns") > -1))
          {
            ControlsToHide.Add("btnClientReturns");
            cmbClientReturns.Enabled = false;
            rfvClientReturns.Enabled = false;
          }
          if (DataComponent.DenyVisible.IndexOf("ClientReturns") > -1) 
          {
            lblClientReturns.Visible = false;
            cmbClientReturns.Visible = false;
            rfvClientReturns.Enabled = false;
          }
          if (OnFieldInit != null)
          {
            OnFieldInit(cmbClientReturns, new EventArgs());
          }
   if (!(DataComponent.DenyVisible.IndexOf("NDS") > -1)) {
    ...
    private void btnSave_Click(object sender, System.EventArgs e)
    {
         if (RowItem == null)
         {
           ...
           {
              TP_MIN_DEFLabelInfo.Text = "Error on create row. ";
              TP_MIN_DEFLabelInfo.ForeColor = System.Drawing.Color.Red;
           }
           return;
         }
       ...
       catch
       {
         if (OnInfo != null)
           ...
           {
              TP_MIN_DEFLabelInfo.Text = "Введите правильную дату в поле Дата акта (например 14.11.1973)";
           }
       }
   RowItem.ActNum = txtActNum.Text;
        try {
        RowItem.ClientReturns = (TP_MIN.enumBoolean)Convert.ToInt32(cmbClientReturns.SelectedValue);
        }catch{}
       try
       {
       ...
       {
         TP_MIN_DEFLabelInfo.Text = "Enter numeric value to field Ставка НДС (example 10)";
         return;
       }
      try
      {
        ...
        {
          TP_MIN_DEFLabelInfo.ForeColor = System.Drawing.Color.Green;
          TP_MIN_DEFLabelInfo.Text = "Row saved";
        }
        if (OnAfterSave != null)
        {
      ...
      {
        TP_MIN_DEFLabelInfo.Text = "Save error. " + Ex.Message;
        TP_MIN_DEFLabelInfo.ForeColor = System.Drawing.Color.Red;
      }
    }
