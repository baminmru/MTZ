...
Begin VB.UserControl vpnVRDGReport_read 
   ClientHeight    =   3600
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   4800
   ...
   Begin VB.Frame pnlVRDGReport 
      ...
      Begin GridEX20.GridEX gridVRDGReport 
         ...
         IntProp7        =   0
         ColumnsCount    =   2
         Column(1)       =   "vpnVRDGReport_read.ctx":0000
         Column(2)       =   "vpnVRDGReport_read.ctx":00C8
         FormatStylesCount=   5
         FormatStyle(1)  =   "vpnVRDGReport_read.ctx":016C
         FormatStyle(2)  =   "vpnVRDGReport_read.ctx":02C8
         FormatStyle(3)  =   "vpnVRDGReport_read.ctx":0378
         FormatStyle(4)  =   "vpnVRDGReport_read.ctx":042C
         FormatStyle(5)  =   "vpnVRDGReport_read.ctx":0504
         ImageCount      =   0
         PrinterProperties=   "vpnVRDGReport_read.ctx":05BC
      End
   End
   ...
   End
End
Attribute VB_Name = "vpnVRDGReport_read"
...
Private Sub opVRDGReport_OpenRef_read(RowItem As Object)
With RowItem
Run_VBOpenRef StartMode:="CONT", ID:=.Report.ID, RowItem:=RowItem
End With
End Sub
Private Sub opVRDGReport_MakeRpt_read(RowItem As Object)
With RowItem
...
If gridVRDGReport.Row > 0 Then
 If gridVRDGReport.RowIndex(gridVRDGReport.Row) > 0 Then
    ...
    bm = gridVRDGReport.RowBookmark(gridVRDGReport.RowIndex(gridVRDGReport.Row))
    Set u = Item.FindRowObject(Right(bm, Len(bm) - 38), Left(bm, 38))
    Set frmVRDGReport_read.Item = u
again:     frmVRDGReport_read.NotFirstTime = False
    frmVRDGReport_read.OnInit
    frmVRDGReport_read.Show vbModal
    If frmVRDGReport_read.OK Then
    On Error Resume Next
    err.Clear
    u.Save
