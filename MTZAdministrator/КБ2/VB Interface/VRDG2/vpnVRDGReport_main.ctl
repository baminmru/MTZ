...
Begin VB.UserControl vpnVRDGReport_main 
   ClientHeight    =   3600
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   4800
   ...
   Begin VB.Frame pnlVRDGReport 
      ...
      Begin GridEX20.GridEX gridVRDGReport 
         ...
         IntProp7        =   0
         ColumnsCount    =   2
         Column(1)       =   "vpnVRDGReport_main.ctx":0000
         Column(2)       =   "vpnVRDGReport_main.ctx":00C8
         FormatStylesCount=   5
         FormatStyle(1)  =   "vpnVRDGReport_main.ctx":016C
         FormatStyle(2)  =   "vpnVRDGReport_main.ctx":02C8
         FormatStyle(3)  =   "vpnVRDGReport_main.ctx":0378
         FormatStyle(4)  =   "vpnVRDGReport_main.ctx":042C
         FormatStyle(5)  =   "vpnVRDGReport_main.ctx":0504
         ImageCount      =   0
         PrinterProperties=   "vpnVRDGReport_main.ctx":05BC
      End
   End
   ...
   End
End
Attribute VB_Name = "vpnVRDGReport_main"
...
Private Sub opVRDGReport_OpenRef_main(RowItem As Object)
With RowItem
Run_VBOpenRef StartMode:="CONT", ID:=.Report.ID, RowItem:=RowItem
End With
End Sub
Private Sub opVRDGReport_MakeRpt_main(RowItem As Object)
With RowItem
...
If gridVRDGReport.Row > 0 Then
 If gridVRDGReport.RowIndex(gridVRDGReport.Row) > 0 Then
    ...
    bm = gridVRDGReport.RowBookmark(gridVRDGReport.RowIndex(gridVRDGReport.Row))
    Set u = Item.FindRowObject(Right(bm, Len(bm) - 38), Left(bm, 38))
    Set frmVRDGReport_main.Item = u
again:     frmVRDGReport_main.NotFirstTime = False
    frmVRDGReport_main.OnInit
    frmVRDGReport_main.Show vbModal
    If frmVRDGReport_main.OK Then
    On Error Resume Next
    err.Clear
    u.Save
