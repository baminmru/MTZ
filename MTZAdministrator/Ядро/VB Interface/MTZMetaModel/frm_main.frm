...
Begin VB.Form frm_main 
   Caption         =   "Спец.: Метамодель системы"
   ClientHeight    =   3090
   ClientLeft      =   60
   ClientTop       =   750
   ClientWidth     =   4680
   ...
   Tag             =   "Card.ICO"
   Begin MTZMetaModelGUI.ctl_main ctl 
      Height          =   1500
      Left            =   0
      TabIndex        =   0
      Top             =   0
...
Private Sub LoadFromSkin()
  ...
  Dim ff As Long, buf As String
  ff = FreeFile
  Open s & "MainForm" & "MTZMetaModel" For Input As #ff
  buf = Input(LOF(ff), ff)
  Close #ff
...
Private Sub SaveToSkin()
  ...
  Dim ff As Long
  ff = FreeFile
  Kill s & "MainForm" & "MTZMetaModel"
  Open s & "MainForm" & "MTZMetaModel" For Output As #ff
  buf = "FormTag:" & Me.Tag
