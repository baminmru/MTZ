...
Begin VB.UserControl vpnUsers_ 
   ClientHeight    =   3600
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   4800
   ...
   Begin VB.Frame pnlUsers 
      ...
      Begin GridEX20.GridEX gridUsers 
         ...
         IntProp7        =   0
         ColumnsCount    =   2
         Column(1)       =   "vpnUsers_.ctx":0000
         Column(2)       =   "vpnUsers_.ctx":00C8
         FormatStylesCount=   5
         FormatStyle(1)  =   "vpnUsers_.ctx":016C
         FormatStyle(2)  =   "vpnUsers_.ctx":02C8
         FormatStyle(3)  =   "vpnUsers_.ctx":0378
         FormatStyle(4)  =   "vpnUsers_.ctx":042C
         FormatStyle(5)  =   "vpnUsers_.ctx":0504
         ImageCount      =   0
         PrinterProperties=   "vpnUsers_.ctx":05BC
      End
...
'панель для работы со строками раздела Пользователи
...
Dim jset As Object 'MTZ_JSetup.Customizer
Set jset = CreateObject("MTZ_JSetup.Customizer")
jset.Preview gridUsers, "Пользователи"
Set jset = Nothing
End Sub
...
If gridUsers.Row > 0 Then
 If gridUsers.RowIndex(gridUsers.Row) > 0 Then
    ...
    bm = gridUsers.RowBookmark(gridUsers.RowIndex(gridUsers.Row))
    Set u = Item.FindRowObject(Right(bm, Len(bm) - 38), Left(bm, 38))
    Set frmUsers_.Item = u
again:     frmUsers_.NotFirstTime = False
    frmUsers_.OnInit
    frmUsers_.Show vbModal
    If frmUsers_.OK Then
    On Error Resume Next
    err.Clear
    u.Save
