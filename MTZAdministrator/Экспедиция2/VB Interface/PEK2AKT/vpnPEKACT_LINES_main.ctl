...
Begin VB.UserControl vpnPEKACT_LINES_main 
   ClientHeight    =   3600
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   4800
   ...
   Begin VB.Frame pnlPEKACT_LINES 
      ...
      Begin GridEX20.GridEX gridPEKACT_LINES 
         ...
         IntProp7        =   0
         ColumnsCount    =   2
         Column(1)       =   "vpnPEKACT_LINES_main.ctx":0000
         Column(2)       =   "vpnPEKACT_LINES_main.ctx":00C8
         FormatStylesCount=   5
         FormatStyle(1)  =   "vpnPEKACT_LINES_main.ctx":016C
         FormatStyle(2)  =   "vpnPEKACT_LINES_main.ctx":02C8
         FormatStyle(3)  =   "vpnPEKACT_LINES_main.ctx":0378
         FormatStyle(4)  =   "vpnPEKACT_LINES_main.ctx":042C
         FormatStyle(5)  =   "vpnPEKACT_LINES_main.ctx":0504
         ImageCount      =   0
         PrinterProperties=   "vpnPEKACT_LINES_main.ctx":05BC
      End
   ...
   End
End
Attribute VB_Name = "vpnPEKACT_LINES_main"
...
If gridPEKACT_LINES.Row > 0 Then
 If gridPEKACT_LINES.RowIndex(gridPEKACT_LINES.Row) > 0 Then
    ...
    bm = gridPEKACT_LINES.RowBookmark(gridPEKACT_LINES.RowIndex(gridPEKACT_LINES.Row))
    Set u = Item.FindRowObject(Right(bm, Len(bm) - 38), Left(bm, 38))
    Set frmPEKACT_LINES_main.Item = u
again:     frmPEKACT_LINES_main.NotFirstTime = False
    frmPEKACT_LINES_main.OnInit
    frmPEKACT_LINES_main.Show vbModal
    If frmPEKACT_LINES_main.OK Then
    On Error Resume Next
    err.Clear
    u.Save
