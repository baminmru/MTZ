...
Begin VB.UserControl PEKACT_DEFfill 
   ClientHeight    =   3600
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   4800
   ...
   End
End
Attribute VB_Name = "PEKACT_DEFfill"
...
If OnInit Then Exit Sub
  ...
  If IsNull(dtpdateCreate) Then
    Item.dateCreate = 0
  Else
    Item.dateCreate = dtpdateCreate.Value
  End If
  If IsNull(dtpdateSend) Then
    Item.dateSend = 0
  Else
    Item.dateSend = dtpdateSend.Value
  End If
  If IsNull(dtpdateReceive) Then
    Item.dateReceive = 0
  Else
    Item.dateReceive = dtpdateReceive.Value
  End If
Item.FIO = txtFIO
