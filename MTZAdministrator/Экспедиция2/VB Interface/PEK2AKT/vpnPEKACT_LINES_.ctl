...
Begin VB.UserControl vpnPEKACT_LINES_ 
   ClientHeight    =   3600
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   4800
   ...
   Begin VB.Frame pnlPEKACT_LINES 
      ...
      Begin GridEX20.GridEX gridPEKACT_LINES 
         ...
         IntProp7        =   0
         ColumnsCount    =   2
         Column(1)       =   "vpnPEKACT_LINES_.ctx":0000
         Column(2)       =   "vpnPEKACT_LINES_.ctx":00C8
         FormatStylesCount=   5
         FormatStyle(1)  =   "vpnPEKACT_LINES_.ctx":016C
         FormatStyle(2)  =   "vpnPEKACT_LINES_.ctx":02C8
         FormatStyle(3)  =   "vpnPEKACT_LINES_.ctx":0378
         FormatStyle(4)  =   "vpnPEKACT_LINES_.ctx":042C
         FormatStyle(5)  =   "vpnPEKACT_LINES_.ctx":0504
         ImageCount      =   0
         PrinterProperties=   "vpnPEKACT_LINES_.ctx":05BC
      End
   ...
   End
End
Attribute VB_Name = "vpnPEKACT_LINES_"
...
If gridPEKACT_LINES.Row > 0 Then
 If gridPEKACT_LINES.RowIndex(gridPEKACT_LINES.Row) > 0 Then
    ...
    bm = gridPEKACT_LINES.RowBookmark(gridPEKACT_LINES.RowIndex(gridPEKACT_LINES.Row))
    Set u = Item.FindRowObject(Right(bm, Len(bm) - 38), Left(bm, 38))
    Set frmPEKACT_LINES_.Item = u
again:     frmPEKACT_LINES_.NotFirstTime = False
    frmPEKACT_LINES_.OnInit
    frmPEKACT_LINES_.Show vbModal
    If frmPEKACT_LINES_.OK Then
    On Error Resume Next
    err.Clear
    u.Save
