VERSION 5.00
Begin VB.Form frmPEKACT_DEF_fill 
   BorderStyle     =   5  'Sizable ToolWindow
   Caption         =   "Описание акта"
   ClientHeight    =   6810
   ClientLeft      =   60
   ClientTop       =   330
   ClientWidth     =   9480
   ...
   Begin VB.CommandButton cmdOK 
      ...
      Left            =   1275
      TabIndex        =   1
      ToolTipText     =   "Акт сверки"
      Top             =   6225
      Width           =   825
   End
   Begin PEK2AKTGUI.PEKACT_DEFfill Panel 
      Height          =   6000
      Left            =   0
      TabIndex        =   0
      Top             =   0
   ...
   End
End
Attribute VB_Name = "frmPEKACT_DEF_fill"
...
Private Sub LoadFormSkin()
  ...
  ff = FreeFile
  Open s & Me.Name & "{0F05FDCC-69EC-4C3D-AE2C-2F9D1DCFD02C}" For Input As #ff
  buf = Input(LOF(ff), ff)
  Close #ff
...
Private Sub SaveToSkin()
  ...
  Dim ff As Long
  ff = FreeFile
  Kill s & Me.Name & "{0F05FDCC-69EC-4C3D-AE2C-2F9D1DCFD02C}"
  Open s & Me.Name & "{0F05FDCC-69EC-4C3D-AE2C-2F9D1DCFD02C}" For Output As #ff
