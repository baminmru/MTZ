VERSION 5.00
Begin VB.Form frmR_main 
   Caption         =   "Сложный маршрут"
   ClientHeight    =   3090
   ClientLeft      =   60
   ClientTop       =   750
   ClientWidth     =   4680
            ...
            'If MsgBox("Закрыть без сохранения?", vbCritical + vbYesNo, Me.Caption) = vbYes Then
                If Not CheckIsOk(item) Then
                    If MsgBox("Поля не заполнены. Удалить документ?", vbCritical + vbYesNo, Me.Caption) = vbYes Then
                        On Error Resume Next
                        item.Manager.DeleteInstance item.ID
                        Host.EraseForm item.ID, "R", ModalMode
                        Set item = Nothing
                        Exit Sub
                    End If
                End If
...
Private Sub LoadFromSkin()
  ...
  Dim ff As Long, buf As String
  ff = FreeFile
  Open s & "MainForm" & "PEK2DP" For Input As #ff
  buf = Input(LOF(ff), ff)
  Close #ff
...
Private Sub SaveToSkin()
  ...
  Dim ff As Long
  ff = FreeFile
  Kill s & "MainForm" & "PEK2DP"
  Open s & "MainForm" & "PEK2DP" For Output As #ff
  buf = "FormTag:" & Me.Tag
