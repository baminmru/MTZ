...
Private Sub LoadFromSkin()
  ...
  Dim ff As Long, buf As String
  ff = FreeFile
  Open s & "MainForm" & "PEK2PQ" For Input As #ff
  buf = Input(LOF(ff), ff)
  Close #ff
...
Private Sub SaveToSkin()
  ...
  Dim ff As Long
  ff = FreeFile
  Kill s & "MainForm" & "PEK2PQ"
  Open s & "MainForm" & "PEK2PQ" For Output As #ff
  buf = "FormTag:" & Me.Tag
