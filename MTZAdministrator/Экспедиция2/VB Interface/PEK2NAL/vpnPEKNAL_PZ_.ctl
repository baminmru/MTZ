...
Begin VB.UserControl vpnPEKNAL_PZ_ 
   ClientHeight    =   3600
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   4800
   ...
   Begin VB.Frame pnlPEKNAL_PZ 
      ...
      Begin GridEX20.GridEX gridPEKNAL_PZ 
         ...
         IntProp7        =   0
         ColumnsCount    =   2
         Column(1)       =   "vpnPEKNAL_PZ_.ctx":0000
         Column(2)       =   "vpnPEKNAL_PZ_.ctx":00C8
         FormatStylesCount=   5
         FormatStyle(1)  =   "vpnPEKNAL_PZ_.ctx":016C
         FormatStyle(2)  =   "vpnPEKNAL_PZ_.ctx":02C8
         FormatStyle(3)  =   "vpnPEKNAL_PZ_.ctx":0378
         FormatStyle(4)  =   "vpnPEKNAL_PZ_.ctx":042C
         FormatStyle(5)  =   "vpnPEKNAL_PZ_.ctx":0504
         ImageCount      =   0
         PrinterProperties=   "vpnPEKNAL_PZ_.ctx":05BC
      End
   ...
   End
End
Attribute VB_Name = "vpnPEKNAL_PZ_"
...
If gridPEKNAL_PZ.Row > 0 Then
 If gridPEKNAL_PZ.RowIndex(gridPEKNAL_PZ.Row) > 0 Then
    ...
    bm = gridPEKNAL_PZ.RowBookmark(gridPEKNAL_PZ.RowIndex(gridPEKNAL_PZ.Row))
    Set u = Item.FindRowObject(Right(bm, Len(bm) - 38), Left(bm, 38))
    Set frmPEKNAL_PZ_.Item = u
again:     frmPEKNAL_PZ_.NotFirstTime = False
    frmPEKNAL_PZ_.OnInit
    frmPEKNAL_PZ_.Show vbModal
    If frmPEKNAL_PZ_.OK Then
    On Error Resume Next
    err.Clear
    u.Save
