...
Begin VB.UserControl vpnPEKNAL_ROWS_ 
   ClientHeight    =   3600
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   4800
   ...
   Begin VB.Frame pnlPEKNAL_ROWS 
      ...
      Begin GridEX20.GridEX gridPEKNAL_ROWS 
         ...
         IntProp7        =   0
         ColumnsCount    =   2
         Column(1)       =   "vpnPEKNAL_ROWS_.ctx":0000
         Column(2)       =   "vpnPEKNAL_ROWS_.ctx":00C8
         FormatStylesCount=   5
         FormatStyle(1)  =   "vpnPEKNAL_ROWS_.ctx":016C
         FormatStyle(2)  =   "vpnPEKNAL_ROWS_.ctx":02C8
         FormatStyle(3)  =   "vpnPEKNAL_ROWS_.ctx":0378
         FormatStyle(4)  =   "vpnPEKNAL_ROWS_.ctx":042C
         FormatStyle(5)  =   "vpnPEKNAL_ROWS_.ctx":0504
         ImageCount      =   0
         PrinterProperties=   "vpnPEKNAL_ROWS_.ctx":05BC
      End
   ...
   End
End
Attribute VB_Name = "vpnPEKNAL_ROWS_"
...
If gridPEKNAL_ROWS.Row > 0 Then
 If gridPEKNAL_ROWS.RowIndex(gridPEKNAL_ROWS.Row) > 0 Then
    ...
    bm = gridPEKNAL_ROWS.RowBookmark(gridPEKNAL_ROWS.RowIndex(gridPEKNAL_ROWS.Row))
    Set u = Item.FindRowObject(Right(bm, Len(bm) - 38), Left(bm, 38))
    Set frmPEKNAL_ROWS_.Item = u
again:     frmPEKNAL_ROWS_.NotFirstTime = False
    frmPEKNAL_ROWS_.OnInit
    frmPEKNAL_ROWS_.Show vbModal
    If frmPEKNAL_ROWS_.OK Then
    On Error Resume Next
    err.Clear
    u.Save
