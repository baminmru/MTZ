...
Begin VB.UserControl vpnPEKNAL_ROWS_read 
   ClientHeight    =   3600
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   4800
   ...
   Begin VB.Frame pnlPEKNAL_ROWS 
      ...
      Begin GridEX20.GridEX gridPEKNAL_ROWS 
         ...
         IntProp7        =   0
         ColumnsCount    =   2
         Column(1)       =   "vpnPEKNAL_ROWS_read.ctx":0000
         Column(2)       =   "vpnPEKNAL_ROWS_read.ctx":00C8
         FormatStylesCount=   5
         FormatStyle(1)  =   "vpnPEKNAL_ROWS_read.ctx":016C
         FormatStyle(2)  =   "vpnPEKNAL_ROWS_read.ctx":02C8
         FormatStyle(3)  =   "vpnPEKNAL_ROWS_read.ctx":0378
         FormatStyle(4)  =   "vpnPEKNAL_ROWS_read.ctx":042C
         FormatStyle(5)  =   "vpnPEKNAL_ROWS_read.ctx":0504
         ImageCount      =   0
         PrinterProperties=   "vpnPEKNAL_ROWS_read.ctx":05BC
      End
   ...
   End
End
Attribute VB_Name = "vpnPEKNAL_ROWS_read"
...
Dim jset As Object 'MTZ_JSetup.Customizer
Set jset = CreateObject("MTZ_JSetup.Customizer")
jset.Preview gridPEKNAL_ROWS, "Раскладка по филиалам"
Set jset = Nothing
End Sub
...
If gridPEKNAL_ROWS.Row > 0 Then
 If gridPEKNAL_ROWS.RowIndex(gridPEKNAL_ROWS.Row) > 0 Then
    ...
    bm = gridPEKNAL_ROWS.RowBookmark(gridPEKNAL_ROWS.RowIndex(gridPEKNAL_ROWS.Row))
    Set u = Item.FindRowObject(Right(bm, Len(bm) - 38), Left(bm, 38))
    Set frmPEKNAL_ROWS_read.Item = u
again:     frmPEKNAL_ROWS_read.NotFirstTime = False
    frmPEKNAL_ROWS_read.OnInit
    frmPEKNAL_ROWS_read.Show vbModal
    If frmPEKNAL_ROWS_read.OK Then
    On Error Resume Next
    err.Clear
    u.Save
