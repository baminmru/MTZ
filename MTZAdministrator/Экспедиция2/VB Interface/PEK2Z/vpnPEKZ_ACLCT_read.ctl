...
Begin VB.UserControl vpnPEKZ_ACLCT_read 
   ClientHeight    =   3600
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   4800
   ...
   Begin VB.Frame pnlPEKZ_ACLCT 
      ...
      Begin GridEX20.GridEX gridPEKZ_ACLCT 
         ...
         IntProp7        =   0
         ColumnsCount    =   2
         Column(1)       =   "vpnPEKZ_ACLCT_read.ctx":0000
         Column(2)       =   "vpnPEKZ_ACLCT_read.ctx":00C8
         FormatStylesCount=   5
         FormatStyle(1)  =   "vpnPEKZ_ACLCT_read.ctx":016C
         FormatStyle(2)  =   "vpnPEKZ_ACLCT_read.ctx":02C8
         FormatStyle(3)  =   "vpnPEKZ_ACLCT_read.ctx":0378
         FormatStyle(4)  =   "vpnPEKZ_ACLCT_read.ctx":042C
         FormatStyle(5)  =   "vpnPEKZ_ACLCT_read.ctx":0504
         ImageCount      =   0
         PrinterProperties=   "vpnPEKZ_ACLCT_read.ctx":05BC
      End
   ...
   End
End
Attribute VB_Name = "vpnPEKZ_ACLCT_read"
...
If gridPEKZ_ACLCT.Row > 0 Then
 If gridPEKZ_ACLCT.RowIndex(gridPEKZ_ACLCT.Row) > 0 Then
    ...
    bm = gridPEKZ_ACLCT.RowBookmark(gridPEKZ_ACLCT.RowIndex(gridPEKZ_ACLCT.Row))
    Set u = item.FindRowObject(Right(bm, Len(bm) - 38), Left(bm, 38))
    Set frmPEKZ_ACLCT_read.item = u
again:     frmPEKZ_ACLCT_read.NotFirstTime = False
    frmPEKZ_ACLCT_read.OnInit
    frmPEKZ_ACLCT_read.Show vbModal
    If frmPEKZ_ACLCT_read.OK Then
    On Error Resume Next
    err.Clear
    u.Save
...
again:       frmPEKZ_ACLCT_read.NotFirstTime = False
      ...
      If frmPEKZ_ACLCT_read.OK Then
        ...
        gridPEKZ_ACLCT.ItemCount = item.PEKZ_ACLCT.Count
        gridPEKZ_ACLCT.Refresh
        gridPEKZ_ACLCT.RefreshGroups
        gridPEKZ_ACLCT.RefreshSort
        gridPEKZ_ACLCT.MoveToBookmark u.ID & "PEKZ_ACLCT"
      Else
        item.PEKZ_ACLCT.Delete u.ID
        item.PEKZ_ACLCT.Remove u.ID
      End If
      Exit Sub
bye:
   MsgBox err.Description, vbOKOnly + vbExclamation, "Создание"
...
Private Sub gridPEKZ_ACLCT_BeforeDeleteEX(ByVal RowIndex As Long, ByVal Bookmark As Variant, ByVal Cancel As GridEX20.JSRetBoolean)
  If gridPEKZ_ACLCT.ItemCount = 0 Then Exit Sub
  Dim u As Object
  Set u = item.FindRowObject("PEKZ_ACLCT", Left(Bookmark, 38))
