...
Begin VB.UserControl vpnpekz_path_main 
   ClientHeight    =   3600
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   4800
   ...
   Begin MTZ_PANEL.HFrames pnlpekz_path 
      ...
      Begin GridEX20.GridEX gridPEKZ_WLIST 
         ...
         IntProp7        =   0
         ColumnsCount    =   2
         Column(1)       =   "vpnpekz_path_main.ctx":0000
         Column(2)       =   "vpnpekz_path_main.ctx":00C8
         FormatStylesCount=   5
         FormatStyle(1)  =   "vpnpekz_path_main.ctx":016C
         FormatStyle(2)  =   "vpnpekz_path_main.ctx":02C8
         FormatStyle(3)  =   "vpnpekz_path_main.ctx":0378
         FormatStyle(4)  =   "vpnpekz_path_main.ctx":042C
         FormatStyle(5)  =   "vpnpekz_path_main.ctx":0504
         ImageCount      =   0
         PrinterProperties=   "vpnpekz_path_main.ctx":05BC
      End
      ...
      Begin GridEX20.GridEX gridpekz_path 
         ...
         IntProp7        =   0
         ColumnsCount    =   2
         Column(1)       =   "vpnpekz_path_main.ctx":0794
         Column(2)       =   "vpnpekz_path_main.ctx":085C
         FormatStylesCount=   5
         FormatStyle(1)  =   "vpnpekz_path_main.ctx":0900
         FormatStyle(2)  =   "vpnpekz_path_main.ctx":0A5C
         FormatStyle(3)  =   "vpnpekz_path_main.ctx":0B0C
         FormatStyle(4)  =   "vpnpekz_path_main.ctx":0BC0
         FormatStyle(5)  =   "vpnpekz_path_main.ctx":0C98
         ImageCount      =   0
         PrinterProperties=   "vpnpekz_path_main.ctx":0D50
      End
   End
   ...
   End
End
Attribute VB_Name = "vpnpekz_path_main"
...
If gridpekz_path.Row > 0 Then
 If gridpekz_path.RowIndex(gridpekz_path.Row) > 0 Then
    ...
    bm = gridpekz_path.RowBookmark(gridpekz_path.RowIndex(gridpekz_path.Row))
    Set u = item.FindRowObject(Right(bm, Len(bm) - 38), Left(bm, 38))
    Set frmpekz_path_main.item = u
again:     frmpekz_path_main.NotFirstTime = False
    frmpekz_path_main.OnInit
    frmpekz_path_main.Show vbModal
    If frmpekz_path_main.OK Then
    On Error Resume Next
    err.Clear
    u.Save
...
Set u.Receiver = u.Application.pekz_def.item(1).ClientTo
...
Set u.Valueedizm = u.Application.FindRowObject("ENTDIC_EDIZM", GetSetting("MUROMETZ", "PEK", "VED"))
  If Not u.Receiver Is Nothing Then
    u.RcvInfo = "Тел.: " & u.Receiver.vrccommoninfo.item(1).Phone & vbCrLf & "Факс: " & u.Receiver.vrccommoninfo.item(1).Fax
  Else
    u.RcvInfo = ""
  End If
u.CalcWithNDS = -1
...
again:       frmpekz_path_main.NotFirstTime = False
      ...
      If frmpekz_path_main.OK Then
        ...
        gridpekz_path.ItemCount = item.pekz_path.Count
        gridpekz_path.Refresh
        gridpekz_path.RefreshGroups
        gridpekz_path.RefreshSort
        gridpekz_path.MoveToBookmark u.ID & "pekz_path"
      Else
        item.pekz_path.Delete u.ID
        item.pekz_path.Remove u.ID
      End If
      Exit Sub
bye:
   MsgBox err.Description, vbOKOnly + vbExclamation, "Создание"
...
Private Sub cmdPEKZ_WLISTEdit_Click()
    ...
    Set frmPEKZ_WLIST_main.item = u
again:     frmPEKZ_WLIST_main.NotFirstTime = False
    frmPEKZ_WLIST_main.OnInit
    frmPEKZ_WLIST_main.Show vbModal
    If frmPEKZ_WLIST_main.OK Then
    On Error Resume Next
    err.Clear
    u.Save
    If err.Number <> 0 Then
...
' получаем номер
  ...
  Dim vl As Integer
  Dim s As String
  Dim nn As Integer, cc As Integer, cr As Integer, i As Integer, ee As Integer
  Dim ppo_num As Integer
  Dim nums As Object


  
  Set nvs = New NamedValues
  Set nv = nvs.Add("Dept", u.Application.pekz_def.item(1).Fillial.ID)
  nv.ValueType = adGUID
  ...
  vl = -1
  Set nv = nvs.Add("NewNum", vl)
  nv.ValueType = adInteger
  nv.ValueDirection = adParamOutput
  
  s = CreateGUID2
  Set nv = nvs.Add("PEKNID", s)
  nv.ValueType = adGUID
  nv.ValueDirection = adParamOutput
  u.Application.MTZSession.Exec "GetPPOCode", nvs
  
  If nvs.item("NewNum").Value > 0 Then

     Set nums = u.Application.Manager.GetInstanceObject(nvs.item("PEKNID").Value)
    'считаем контрольный разряд
    ppo_num = nvs.item("NewNum").Value
 
    ' строим правильный номер с учетом настроек
         s = Right("000000" & nums.PEKD_NZONE.item(1).POIDX, 6) & _
      IIf(GetSetting("MUROMETZ", "PEK", "DateSHCODE", "") <> "", _
      Right("00" & GetSetting("MUROMETZ", "PEK", "DateSHCODE", ""), 2), _
      Right("00" & (DateDiff("M", "01/01/2000", Date) + 1), 2)) _
      & _
      Right("00000" & nvs.item("NewNum").Value, 5)

    nn = 0
    For i = 1 To 13
      nn = nn + val("0" & Mid(s, i, 1))
    Next
    
    cc = 0
    For i = 2 To 12
      cc = cc + val("0" & Mid(s, i, 1))
    Next
...
again:       frmPEKZ_WLIST_main.NotFirstTime = False
      ...
      If frmPEKZ_WLIST_main.OK Then
        ...
        gridPEKZ_WLIST.ItemCount = gu.PEKZ_WLIST.Count
        gridPEKZ_WLIST.Refresh
        gridPEKZ_WLIST.RefreshGroups
        gridPEKZ_WLIST.RefreshSort
        gridPEKZ_WLIST.MoveToBookmark u.ID & "PEKZ_WLIST"
      Else
        gu.PEKZ_WLIST.Delete u.ID
        gu.PEKZ_WLIST.Remove u.ID
      End If
Exit Sub
bye:
MsgBox err.Description, vbOKOnly + vbExclamation, "Создание"
