...
Begin VB.UserControl vpnPEKZ_ADLVR_main 
   ClientHeight    =   3600
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   4800
   ...
   Begin VB.Frame pnlPEKZ_ADLVR 
      ...
      Begin GridEX20.GridEX gridPEKZ_ADLVR 
         ...
         IntProp7        =   0
         ColumnsCount    =   2
         Column(1)       =   "vpnPEKZ_ADLVR_main.ctx":0000
         Column(2)       =   "vpnPEKZ_ADLVR_main.ctx":00C8
         FormatStylesCount=   5
         FormatStyle(1)  =   "vpnPEKZ_ADLVR_main.ctx":016C
         FormatStyle(2)  =   "vpnPEKZ_ADLVR_main.ctx":02C8
         FormatStyle(3)  =   "vpnPEKZ_ADLVR_main.ctx":0378
         FormatStyle(4)  =   "vpnPEKZ_ADLVR_main.ctx":042C
         FormatStyle(5)  =   "vpnPEKZ_ADLVR_main.ctx":0504
         ImageCount      =   0
         PrinterProperties=   "vpnPEKZ_ADLVR_main.ctx":05BC
      End
   ...
   End
End
Attribute VB_Name = "vpnPEKZ_ADLVR_main"
...
If gridPEKZ_ADLVR.Row > 0 Then
 If gridPEKZ_ADLVR.RowIndex(gridPEKZ_ADLVR.Row) > 0 Then
    ...
    bm = gridPEKZ_ADLVR.RowBookmark(gridPEKZ_ADLVR.RowIndex(gridPEKZ_ADLVR.Row))
    Set u = item.FindRowObject(Right(bm, Len(bm) - 38), Left(bm, 38))
    Set frmPEKZ_ADLVR_main.item = u
again:     frmPEKZ_ADLVR_main.NotFirstTime = False
    frmPEKZ_ADLVR_main.OnInit
    frmPEKZ_ADLVR_main.Show vbModal
    If frmPEKZ_ADLVR_main.OK Then
    On Error Resume Next
    err.Clear
    u.Save
...
On Error Resume Next
Set u.theFilial = u.Application.FindRowObject("PEKD_DEPT", GetSetting("MUROMETZ", "PEK", "CurrentFIL", ""))
...
again:       frmPEKZ_ADLVR_main.NotFirstTime = False
      ...
      If frmPEKZ_ADLVR_main.OK Then
        ...
        gridPEKZ_ADLVR.ItemCount = item.PEKZ_ADLVR.Count
        gridPEKZ_ADLVR.Refresh
        gridPEKZ_ADLVR.RefreshGroups
        gridPEKZ_ADLVR.RefreshSort
        gridPEKZ_ADLVR.MoveToBookmark u.ID & "PEKZ_ADLVR"
      Else
        item.PEKZ_ADLVR.Delete u.ID
        item.PEKZ_ADLVR.Remove u.ID
      End If
      Exit Sub
bye:
   MsgBox err.Description, vbOKOnly + vbExclamation, "Создание"
...
Private Sub gridPEKZ_ADLVR_BeforeDeleteEX(ByVal RowIndex As Long, ByVal Bookmark As Variant, ByVal Cancel As GridEX20.JSRetBoolean)
  If gridPEKZ_ADLVR.ItemCount = 0 Then Exit Sub
  Dim u As Object
  Set u = item.FindRowObject("PEKZ_ADLVR", Left(Bookmark, 38))
