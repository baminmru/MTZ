...
Begin VB.UserControl pekz_pathmain 
   ClientHeight    =   3600
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   4800
   ...
   End
End
Attribute VB_Name = "pekz_pathmain"
...
Private Sub cmdTransSupplier_CLick()
  On Error Resume Next
     If txtTransSupplier.Tag = "" Then
       cmdTransSupplier_MenuClick "Выбрать"
     Else
...
Private Sub cmdTransSupplier_MenuClick(ByVal sCaption As String)
  ...
  If sCaption = "Выбрать" Then
        ...
        If OK Then
          txtTransSupplier.Tag = Left(ID, 38)
          txtTransSupplier = brief
        End If
  End If
  If sCaption = "Создать" Then
     On Error Resume Next
        ...
        If Not obj Is Nothing Then
          ...
          If (Not obj Is Nothing) Then
              txtTransSupplier.Tag = obj.ID
              txtTransSupplier = obj.brief
          Else
              txtTransSupplier.Tag = ""
              txtTransSupplier = ""
          End If
...
Private Sub cmdTheDirection_CLick()
  On Error Resume Next
        Dim ID As String, brief As String
        If Item.Application.Manager.GetReferenceDialogEx2("PEKD_DIRECTION", ID, brief, , , "PEKD_DIRECTION_Dept_ID='" & txtPathFilial.Tag & "'") Then
          txtTheDirection.Tag = Left(ID, 38)
          txtTheDirection = brief
        End If
End Sub
Private Sub cmdTheDirection_MenuClick(ByVal sCaption As String)
          ...
          txtTheDirection = ""
End Sub
Private Sub txtPPONum_Validate(cancel As Boolean)
If txtPPONum.Text <> "" Then
 On Error Resume Next
  If Not IsNumeric(txtPPONum.Text) Then
     cancel = True
     MsgBox "Ожидалось число", vbOKOnly + vbExclamation, "Внимание"
     txtPPONum.SetFocus
  ElseIf Val(txtPPONum.Text) <> CLng(Val(txtPPONum.Text)) Then
     cancel = True
...
End If
End Sub
Private Sub txtPPONum_KeyPess(KeyAscii As Integer)
Dim s As String
s = "0123456789.,-" & Chr(8)
...
Private Sub cmdDestStation_CLick()
  On Error Resume Next
        Dim ID As String, brief As String
        If Item.Application.Manager.GetReferenceDialogEx2("PEKD_DIRSTATION", ID, brief, , , IIf(txtTheDirection.Tag = "", "PEKD_DIRECTIONid is null", "PEKD_DIRECTIONid='" & txtTheDirection.Tag & "'")) Then
          txtDestStation.Tag = Left(ID, 38)
          txtDestStation = brief
        End If
End Sub
...
Private Sub cmdPPODescription_CLick()
  On Error Resume Next
        Dim ID As String, brief As String
        If Item.Application.Manager.GetReferenceDialogEx2("PEKD_POSTTYPE", ID, brief) Then
          txtPPODescription.Tag = Left(ID, 38)
          txtPPODescription = brief
        End If
End Sub
...
Private Sub cmdReceiver_MenuClick(ByVal sCaption As String)
  ...
  If sCaption = "Создать" Then
     On Error Resume Next
        ...
        If Not obj Is Nothing Then
          ...
          Set obj = Item.Application.Manager.GetInstanceObject(obj.ID)
          If (Not obj Is Nothing) Then
              txtReceiver.Tag = obj.ID
              txtReceiver = obj.brief
          Else
              txtReceiver.Tag = ""
              txtReceiver = ""
          End If
  ...
  End If
End Sub
Private Sub txtPPOWeight_Validate(cancel As Boolean)
If txtPPOWeight.Text <> "" Then
 On Error Resume Next
  If Not IsNumeric(txtPPOWeight.Text) Then
     cancel = True
     MsgBox "Ожидалось число", vbOKOnly + vbExclamation, "Внимание"
     txtPPOWeight.SetFocus
  ElseIf Val(txtPPOWeight.Text) < -922337203685478# Or Val(txtPPOWeight.Text) > 922337203685478# Then
     cancel = True
...
End If
End Sub
Private Sub txtPPOWeight_KeyPess(KeyAscii As Integer)
Dim s As String
s = "0123456789.,-" & Chr(8)
If InStr(s, Chr(KeyAscii)) > 0 Then Exit Sub
KeyAscii = 0
...
Private Sub cmdWeightEDIZM_MenuClick(ByVal sCaption As String)
          ...
          txtWeightEDIZM = ""
End Sub
Private Sub txtPPOVol_Validate(cancel As Boolean)
If txtPPOVol.Text <> "" Then
 On Error Resume Next
  If Not IsNumeric(txtPPOVol.Text) Then
     cancel = True
     MsgBox "Ожидалось число", vbOKOnly + vbExclamation, "Внимание"
     txtPPOVol.SetFocus
  ElseIf Val(txtPPOVol.Text) < -922337203685478# Or Val(txtPPOVol.Text) > 922337203685478# Then
     cancel = True
     MsgBox "Значение вне допустимого диапазона", vbOKOnly + vbExclamation, "Внимание"
     txtPPOVol.SetFocus
  End If
End If
End Sub
Private Sub txtPPOVol_KeyPess(KeyAscii As Integer)
Dim s As String
s = "0123456789.,-" & Chr(8)
If InStr(s, Chr(KeyAscii)) > 0 Then Exit Sub
KeyAscii = 0
...
Private Sub cmdtrainNo_CLick()
  On Error Resume Next
        Dim ID As String, brief As String
        If Item.Application.Manager.GetReferenceDialogEx2("PEKD_TRAINS", ID, brief, , , IIf(txtTheDirection.Tag = "", "PEKD_DIRECTIONid is null", "PEKD_DIRECTIONid='" & txtTheDirection.Tag & "'")) Then
          txttrainNo.Tag = Left(ID, 38)
          txttrainNo = brief
        End If
End Sub
...
Private Sub cmdInfoSendType_CLick()
  On Error Resume Next
        Dim ID As String, brief As String
        If Item.Application.Manager.GetReferenceDialogEx2("PEKD_INFOSENDTYPE", ID, brief) Then
          txtInfoSendType.Tag = Left(ID, 38)
          txtInfoSendType = brief
        End If
End Sub
...
End If
 LoadBtnPictures cmdTransSupplier, cmdTransSupplier.Tag
  ...
  cmdTransSupplier.AddMenu "Создать"
  cmdTransSupplier.AddMenu "Выбрать"
...
End If
 LoadBtnPictures cmdReceiver, cmdReceiver.Tag
  ...
  cmdReceiver.AddMenu "Создать"
  cmdReceiver.AddMenu "Выбрать"
...
End If
If txtTransSupplier.Tag <> "" Then
  Set Item.TransSupplier = Item.Application.Manager.GetInstanceObject(txtTransSupplier.Tag)
Else
  Set Item.TransSupplier = Nothing
End If
If txtTheDirection.Tag <> "" Then
  Set Item.TheDirection = Item.Application.FindRowObject("PEKD_DIRECTION", txtTheDirection.Tag)
Else
  Set Item.TheDirection = Nothing
End If
Item.PPONum = CDbl(txtPPONum)
If txtDestStation.Tag <> "" Then
  Set Item.DestStation = Item.Application.FindRowObject("PEKD_DIRSTATION", txtDestStation.Tag)
Else
  Set Item.DestStation = Nothing
End If
If txtPPODescription.Tag <> "" Then
  Set Item.PPODescription = Item.Application.FindRowObject("PEKD_POSTTYPE", txtPPODescription.Tag)
Else
  Set Item.PPODescription = Nothing
End If
If txtReceiver.Tag <> "" Then
  Set Item.Receiver = Item.Application.Manager.GetInstanceObject(txtReceiver.Tag)
Else
  Set Item.Receiver = Nothing
End If
Item.PPOWeight = CDbl(txtPPOWeight)
...
End If
  If IsNull(dtpPathPlanedDate) Then
    Item.PathPlanedDate = 0
  Else
    Item.PathPlanedDate = dtpPathPlanedDate.Value
  End If
  If IsNull(dtpRcvWaitDate) Then
    Item.RcvWaitDate = 0
  Else
    Item.RcvWaitDate = dtpRcvWaitDate.Value
  End If
If txttrainNo.Tag <> "" Then
  Set Item.trainNo = Item.Application.FindRowObject("PEKD_TRAINS", txttrainNo.Tag)
Else
  Set Item.trainNo = Nothing
End If
Item.vagNo = txtvagNo
  If IsNull(dtpSendDate) Then
    Item.SendDate = 0
  Else
    Item.SendDate = dtpSendDate.Value
  End If
  ...
  End If
Item.PassFIO = txtPassFIO
  If IsNull(dtpPassDate) Then
    Item.PassDate = 0
  Else
    Item.PassDate = dtpPassDate.Value
  End If
Item.RcvFIO = txtRcvFIO
  If IsNull(dtpRcvDate) Then
    Item.RcvDate = 0
  Else
    Item.RcvDate = dtpRcvDate.Value
  End If
Item.InforRcvFIO = txtInforRcvFIO
  If IsNull(dtpInfoDate) Then
    Item.InfoDate = 0
  Else
    Item.InfoDate = dtpInfoDate.Value
  End If
If txtInfoSendType.Tag <> "" Then
  Set Item.InfoSendType = Item.Application.FindRowObject("PEKD_INFOSENDTYPE", txtInfoSendType.Tag)
Else
  Set Item.InfoSendType = Nothing
End If
