...
Begin VB.UserControl vpnPEKZ_ACLCT_prnt 
   ClientHeight    =   3600
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   4800
   ...
   Begin VB.Frame pnlPEKZ_ACLCT 
      ...
      Begin GridEX20.GridEX gridPEKZ_ACLCT 
         ...
         IntProp7        =   0
         ColumnsCount    =   2
         Column(1)       =   "vpnPEKZ_ACLCT_prnt.ctx":0000
         Column(2)       =   "vpnPEKZ_ACLCT_prnt.ctx":00C8
         FormatStylesCount=   5
         FormatStyle(1)  =   "vpnPEKZ_ACLCT_prnt.ctx":016C
         FormatStyle(2)  =   "vpnPEKZ_ACLCT_prnt.ctx":02C8
         FormatStyle(3)  =   "vpnPEKZ_ACLCT_prnt.ctx":0378
         FormatStyle(4)  =   "vpnPEKZ_ACLCT_prnt.ctx":042C
         FormatStyle(5)  =   "vpnPEKZ_ACLCT_prnt.ctx":0504
         ImageCount      =   0
         PrinterProperties=   "vpnPEKZ_ACLCT_prnt.ctx":05BC
      End
   ...
   End
End
Attribute VB_Name = "vpnPEKZ_ACLCT_prnt"
...
If gridPEKZ_ACLCT.Row > 0 Then
 If gridPEKZ_ACLCT.RowIndex(gridPEKZ_ACLCT.Row) > 0 Then
    ...
    bm = gridPEKZ_ACLCT.RowBookmark(gridPEKZ_ACLCT.RowIndex(gridPEKZ_ACLCT.Row))
    Set u = item.FindRowObject(Right(bm, Len(bm) - 38), Left(bm, 38))
    Set frmPEKZ_ACLCT_prnt.item = u
again:     frmPEKZ_ACLCT_prnt.NotFirstTime = False
    frmPEKZ_ACLCT_prnt.OnInit
    frmPEKZ_ACLCT_prnt.Show vbModal
    If frmPEKZ_ACLCT_prnt.OK Then
    On Error Resume Next
    err.Clear
    u.Save
