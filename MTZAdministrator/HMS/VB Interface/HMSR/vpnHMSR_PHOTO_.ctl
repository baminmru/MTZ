...
Begin VB.UserControl vpnHMSR_PHOTO_ 
   ClientHeight    =   3600
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   4800
   ...
   Begin VB.Frame pnlHMSR_PHOTO 
      ...
      Begin GridEX20.GridEX gridHMSR_PHOTO 
         ...
         IntProp7        =   0
         ColumnsCount    =   2
         Column(1)       =   "vpnHMSR_PHOTO_.ctx":0000
         Column(2)       =   "vpnHMSR_PHOTO_.ctx":00C8
         FormatStylesCount=   5
         FormatStyle(1)  =   "vpnHMSR_PHOTO_.ctx":016C
         FormatStyle(2)  =   "vpnHMSR_PHOTO_.ctx":02C8
         FormatStyle(3)  =   "vpnHMSR_PHOTO_.ctx":0378
         FormatStyle(4)  =   "vpnHMSR_PHOTO_.ctx":042C
         FormatStyle(5)  =   "vpnHMSR_PHOTO_.ctx":0504
         ImageCount      =   0
         PrinterProperties=   "vpnHMSR_PHOTO_.ctx":05BC
      End
   ...
   End
End
Attribute VB_Name = "vpnHMSR_PHOTO_"
...
If gridHMSR_PHOTO.Row > 0 Then
 If gridHMSR_PHOTO.RowIndex(gridHMSR_PHOTO.Row) > 0 Then
    ...
    bm = gridHMSR_PHOTO.RowBookmark(gridHMSR_PHOTO.RowIndex(gridHMSR_PHOTO.Row))
    Set u = item.FindRowObject(Right(bm, Len(bm) - 38), Left(bm, 38))
    Set frmHMSR_PHOTO_.item = u
again:     frmHMSR_PHOTO_.NotFirstTime = False
    frmHMSR_PHOTO_.OnInit
    frmHMSR_PHOTO_.Show vbModal
    If frmHMSR_PHOTO_.OK Then
    On Error Resume Next
    err.Clear
    u.Save
