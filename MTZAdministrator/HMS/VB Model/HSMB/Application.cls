...
'корневой интерфейс документа (Здание)
...
Private Const m_TypeName As String = "HSMB"
...
End Enum
Private m_HMSB_DEF As HMSB_DEF_col
...
'Даные раздела Описание здания
'Parameters:
'[IN]   Refresh , тип параметра: boolean = true  - ...
'Returns:
'  объект класса HMSB_DEF_col
'  ,или Nothing
'See Also:
'Example:
' dim variable as HMSB_DEF_col
' Set variable = me. HMSB_DEF (...параметры...)
Public Property Get HMSB_DEF(Optional ByVal Refresh As Boolean = True) As HMSB_DEF_col
 If m_HMSB_DEF Is Nothing Then
   Set m_HMSB_DEF = New HMSB_DEF_col
   Set m_HMSB_DEF.Parent = Me
   Set m_HMSB_DEF.Application = Me.Application
   If Refresh Then m_HMSB_DEF.Refresh
 End If
 Set HMSB_DEF = m_HMSB_DEF
End Property
...
Public Sub CloseClass()
 ...
 Set findCash = Nothing
  ...
  If Not m_HMSB_DEF Is Nothing Then
    m_HMSB_DEF.CloseClass
    Set m_HMSB_DEF = Nothing
  End If
...
Public Function FindObject(ByVal StrID As String, ByVal InstID As String) As Object
 ...
 If m_FindObject Is Nothing Then
   Set m_FindObject = HMSB_DEF.FindObject(StrID, InstID)
 End If
...
Set e_list = anode.selectNodes("HMSB_DEF_COL")
  HMSB_DEF.XMLLoad e_list, LoadMode
End Sub
...
End If
  If mode = "" Then
If CollectionName = "" Or CollectionName = "HMSB_DEF" Then
  If Mask <> "" Then
    If Application.MTZSession.IsDescendant(Mask, "HMSB_DEF") Then
    If key = "" Then
       Set node = tree.nodes.Add(, 0, CreateGUID2() & id & "HMSB_DEF", "Описание здания")
    Else
       Set node = tree.nodes.Add(key, 4, CreateGUID2() & id & "HMSB_DEF", "Описание здания")
    End If
       node.Tag = "HMSB_DEF"
       ' node for <+> item
...
If mode = "" Then
 ...
 Case "HMSB_DEF"
    ...
    For i = 1 To Item.HMSB_DEF.Count
    nid = CreateGUID2() & Item.HMSB_DEF.Item(i).id
    tree.nodes.Add(key, 4, nid, Item.HMSB_DEF.Item(i).Brief(True)).Tag = "Row"
    Next
 End Select
End If ' mode=""
End Sub
