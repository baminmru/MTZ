...
Begin VB.UserControl vpnosDic_EDIZM_ 
   ClientHeight    =   3600
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   4800
   ...
   Begin VB.Frame pnlosDic_EDIZM 
      ...
      Begin GridEX20.GridEX gridosDic_EDIZM 
         ...
         IntProp7        =   0
         ColumnsCount    =   2
         Column(1)       =   "vpnosDic_EDIZM_.ctx":0000
         Column(2)       =   "vpnosDic_EDIZM_.ctx":00C8
         FormatStylesCount=   5
         FormatStyle(1)  =   "vpnosDic_EDIZM_.ctx":016C
         FormatStyle(2)  =   "vpnosDic_EDIZM_.ctx":02C8
         FormatStyle(3)  =   "vpnosDic_EDIZM_.ctx":0378
         FormatStyle(4)  =   "vpnosDic_EDIZM_.ctx":042C
         FormatStyle(5)  =   "vpnosDic_EDIZM_.ctx":0504
         ImageCount      =   0
         PrinterProperties=   "vpnosDic_EDIZM_.ctx":05BC
      End
   ...
   End
End
Attribute VB_Name = "vpnosDic_EDIZM_"
...
If gridosDic_EDIZM.Row > 0 Then
 If gridosDic_EDIZM.RowIndex(gridosDic_EDIZM.Row) > 0 Then
    ...
    bm = gridosDic_EDIZM.RowBookmark(gridosDic_EDIZM.RowIndex(gridosDic_EDIZM.Row))
    Set u = Item.FindRowObject(Right(bm, Len(bm) - 38), Left(bm, 38))
    Set frmosDic_EDIZM_.Item = u
again:     frmosDic_EDIZM_.NotFirstTime = False
    frmosDic_EDIZM_.OnInit
    frmosDic_EDIZM_.Show vbModal
    If frmosDic_EDIZM_.OK Then
    On Error Resume Next
    err.Clear
    u.Save
