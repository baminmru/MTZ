...
Begin VB.UserControl vpnosDic_srvtype_admi 
   ClientHeight    =   3600
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   4800
   ...
   Begin VB.Frame pnlosDic_srvtype 
      ...
      Begin GridEX20.GridEX gridosDic_srvtype 
         ...
         IntProp7        =   0
         ColumnsCount    =   2
         Column(1)       =   "vpnosDic_srvtype_admi.ctx":0000
         Column(2)       =   "vpnosDic_srvtype_admi.ctx":00C8
         FormatStylesCount=   5
         FormatStyle(1)  =   "vpnosDic_srvtype_admi.ctx":016C
         FormatStyle(2)  =   "vpnosDic_srvtype_admi.ctx":02C8
         FormatStyle(3)  =   "vpnosDic_srvtype_admi.ctx":0378
         FormatStyle(4)  =   "vpnosDic_srvtype_admi.ctx":042C
         FormatStyle(5)  =   "vpnosDic_srvtype_admi.ctx":0504
         ImageCount      =   0
         PrinterProperties=   "vpnosDic_srvtype_admi.ctx":05BC
      End
   ...
   Begin VB.Menu mnuPopuposDic_srvtype 
      Caption         =   "Меню для Тип абонемента"
      Visible         =   0   'False
   ...
   End
End
Attribute VB_Name = "vpnosDic_srvtype_admi"
...
Dim jset As Object 'MTZ_JSetup.Customizer
Set jset = CreateObject("MTZ_JSetup.Customizer")
jset.Preview gridosDic_srvtype, "Тип абонемента"
Set jset = Nothing
End Sub
...
If gridosDic_srvtype.Row > 0 Then
 If gridosDic_srvtype.RowIndex(gridosDic_srvtype.Row) > 0 Then
    ...
    bm = gridosDic_srvtype.RowBookmark(gridosDic_srvtype.RowIndex(gridosDic_srvtype.Row))
    Set u = Item.FindRowObject(Right(bm, Len(bm) - 38), Left(bm, 38))
    Set frmosDic_srvtype_admi.Item = u
again:     frmosDic_srvtype_admi.NotFirstTime = False
    frmosDic_srvtype_admi.OnInit
    frmosDic_srvtype_admi.Show vbModal
    If frmosDic_srvtype_admi.OK Then
    On Error Resume Next
    err.Clear
    u.Save
...
Private Sub gridosDic_srvtype_BeforeDeleteEX(ByVal RowIndex As Long, ByVal Bookmark As Variant, ByVal Cancel As GridEX20.JSRetBoolean)
  If gridosDic_srvtype.ItemCount = 0 Then Exit Sub
  Dim u As Object
  Set u = Item.FindRowObject("osDic_srvtype", Left(Bookmark, 38))
