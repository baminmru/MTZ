...
Begin VB.Form frmDocFolder 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Создать ярлык для документа"
   ClientHeight    =   2655
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   5010
   Icon            =   "frmDocFolder.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   2655
   ...
   End
End
Attribute VB_Name = "frmDocFolder"
...
If Not item.WFFD_ATTR1.item(1).Folder Is Nothing Then
  txtFolder.Tag = item.WFFD_ATTR1.item(1).Folder.ID
  txtFolder = item.WFFD_ATTR1.item(1).Folder.Brief
Else
  txtFolder.Tag = ""
  txtFolder = ""
End If
...
If txtFolder.Tag <> "" Then
  Set item.WFFD_ATTR1.item(1).Folder = item.Application.FindRowObject("Folder", txtFolder.Tag)
Else
  Set item.WFFD_ATTR1.item(1).Folder = Nothing
End If
