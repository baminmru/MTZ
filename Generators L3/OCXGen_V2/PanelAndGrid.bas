Attribute VB_Name = "PanelAndGrid"
Option Explicit

Sub MakePanelAndGrid(pctl As ControlData, ot As OBJECTTYPE, p As PART, Child As PART, mode As String, body As String, tsClick As String, tsInit As String)
  On Error GoTo bye
  ...
  body = body & vbCrLf & "on error resume next"
  body = body & vbCrLf & "  if  " & lctl.name & ".item is nothing Then " & rctl.name & ".itemcount=0: Exit Sub"
  ...
  body = body & vbCrLf & "   " & lctl.name & ".item." & Child.name & ".preparegrid " & rctl.name
  ...
  If AllowAdd Then
    ...
    body = body & vbCrLf & "   Set gu = " & lctl.name & ".item"
    body = body & vbCrLf & "   Dim u As Object"
    ...
    If Child.AddBehaivor = PartAddBehaivor_AddForm Then
      ...
      body = body & vbCrLf & "        " & rctl.name & ".ItemCount = Item." & Child.name & ".Count"
  ...
  If AllowEdit Then
    body = body & vbCrLf & "    If " & lctl.name & ".Item IS NOTHING Then Exit Sub"
    body = body & vbCrLf & "    Dim u As Object"
    body = body & vbCrLf & "    Set u = " & lctl.name & ".Item." & Child.name & ".Add()"
