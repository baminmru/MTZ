    ...
    If (st.FIELD.item(i).FIELDTYPE.TypeStyle <> TypeStyle_Element_oformleniy) Then
      ...
      If f.FIELDTYPE.TypeStyle = TypeStyle_Ssilka Then
        If f.ReferenceType = ReferenceType_Na_istocnik_dannih Then
          txt = txt & vbCrLf & "   if obj." & f.name & " = """" then"
          txt = txt & vbCrLf & "    Values.Value(" & i & ") = ""Не задано"""
          txt = txt & vbCrLf & "   else "
          txt = txt & vbCrLf & "     Dim XMLDoc" & f.name & " As New DOMDocument"
          txt = txt & vbCrLf & "     On Error Resume Next"
          txt = txt & vbCrLf & "     Call XMLDoc" & f.name & ".loadXML(obj." & f.name & ")"
          txt = txt & vbCrLf & "     If (Err.Number = 0 And XMLDoc" & f.name & ".parseError.errorCode = 0) Then"
          txt = txt & vbCrLf & "           Dim node" & f.name & " As MSXML2.IXMLDOMNode"
          'txt = txt & vbCrLf & "           txt" & f.name & ".Tag = obj." & f.name & ""
          txt = txt & vbCrLf & "           For Each node" & f.name & " In XMLDoc" & f.name & ".childNodes.item(0).childNodes"
          txt = txt & vbCrLf & "             If (node" & f.name & ".baseName = ""Brief"") Then"
          txt = txt & vbCrLf & "              Values.Value(" & i & ") = node" & f.name & ".Text"
          txt = txt & vbCrLf & "              Exit For"
          txt = txt & vbCrLf & "             End If"
          txt = txt & vbCrLf & "           Next"
          txt = txt & vbCrLf & "      End if"
          txt = txt & vbCrLf & "   end if "
